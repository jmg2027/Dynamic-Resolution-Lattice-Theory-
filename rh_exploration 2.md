...
| Born rule P=|⟨ψ|ψ'⟩|² from ℂ | Paper 2, Thm 4.1 | **Theorem** |
...
2. Its zeros lie in Re(s) = 1/2 ± O(1/√N) (should follow from Paper 1)
3. Z_N(s) → ζ(s) in some appropriate limit
4. The GUE statistics of Z_N match those of ζ
...
2. Zeros of Z_N(s) satisfy Re(s) = 1/2 ± O(1/√N) [from Paper 1 spectral gap]
3. Self-contradiction theorem: δ = 0 ⟺ N = ∞ ⟺ Tr diverges
4. GUE membership: β = 2 forced by ℂ [from Paper 2]

### What can be conjectured (with evidence):
5. Z_N(s) → ζ(s) in appropriate limit
6. Montgomery pair correlation follows from GUE membership

### What is declared (meta-mathematical):
7. RH as a limit theorem / shadow theorem — the exact equality Re(s) = 1/2 is an idealization of a finite structure's O(1/√N) bound
...
2. Oscillatory Dirichlet series Σ e^{iθ}/n^s with uniform θ
3. Convergence boundary = Re(s) = 1/2 (Halász-Harper, known theorem)
4. This is the CLT: Var = Σ1/k^{2σ}, critical at σ = 1/2
5. Born rule |z|² (from ℂ) makes variance quadratic → determines the exponent
...
### A.5 Two Boundaries Theorem (Session 2, final)

**The key insight:** σ_stat = 1/2 (CLT, universal) and σ_geom = 1/n_K (phase equipartition, K-dependent) coincide **only for K = ℂ**.

| K | σ_stat | σ_geom | Match? |
|---|--------|--------|--------|
| ℝ | 1/2 | 1 | No |
| ℂ | 1/2 | 1/2 | **Yes** |
| ℍ | 1/2 | 1/4 | No |

This is a theorem (trivially: 1/2 = 1/n iff n=2 iff K=ℂ). But the meaning is deep: ℂ is the unique algebra where the Dirichlet series convergence boundary equals the phase energy equipartition.

Formalized in: two_boundaries_theorem.md


### A.6 Doubly Irreducible Number Theorem

**Key result:** 2 is the unique natural number that is both an additive atom AND an extension atom over ℝ.

- Additive atoms: {2, 3} (can't write as a+b with a,b≥2)
- Extension atoms over ℝ: {2} (degrees of irreducible real polynomials, by FTA)
- Intersection: {2}

This closes the gap: n_T = 2 and dim_ℝ(ℂ) = 2 are the same "2" because 2 is doubly irreducible. The (3,2) asymmetry is: 3 is singly irreducible (additive only), 2 is doubly irreducible (additive + algebraic).

Formalized in: doubly_irreducible.md
