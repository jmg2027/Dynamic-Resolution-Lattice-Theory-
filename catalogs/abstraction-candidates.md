...
|----|-------|:------:|--------------|
| L2 | `h_components_{α,β}` 4-sibling | **DONE** | `LeibnizDecomp.lean` (8 PURE helpers); 4 sites collapsed. -147 lines net. |
| N7 | `caseElement` Prism truth-table generalisation | **DONE** | `caseElement_preview_{self,other}` (2 PURE); 4 truth-table theorems as one-line corollaries. |
| N8 | `NatHelper.mul_left_comm` adoption | **DONE** | 19 sites across 3 files (CutSumOne ×16, CutMidSelf ×2, Euler ×3). |
| N9 | `Nat.add_right_comm` adoption | **DONE** | 6 sites across 7 files; helper was Lean-core PURE on its own. |
| Sub-2 | Tree slash-arm prologue helper | **DONE** | `Tree.canonical_slash_decompose` + `canonicalBy_slash_decompose`; 5 sites refactored (term-level, not macro). |

## §3 — Mid-size consolidations

| ID | Title | Status | Notes |
|----|-------|:------:|-------|
| L1 | LeibnizAlgLift 4-sibling | **DONE (full)** | β-side: `LeibnizAlgLiftBeta.leibniz_via_β_decomp_general`.  α-side: `LeibnizAlgLiftAlpha.leibniz_via_α_decomp_general` with `castA/castB` `Fin.cast` plumbing for the Nat.add asymmetry.  All 4 siblings 1-line corollaries.  Methodology: `seed/THEOREM_METHODOLOGY_SUITE.md` §TH-4. |
| C  | CutSumOne 8-sibling 3-component template | **DONE** | `cutSum_constCut_at (a₁ b₁ a₂ b₂ X Y m k) (forward) (backward)` extracts the bidirectional iff opener + `decide_eq_true` closer.  All 8 substantive `cutSum_*` refactored; 478→337 lines. |

## §4 — Smaller consolidations

| ID | Title | Status | Notes |
|----|-------|:------:|-------|
| L3 | Pisano Predictor extension steps | **DONE** | `pisano_period_lift {p pf N f}` PURE template absorbs per-prime `intro k; rw [h_p]; exact period_n k` boilerplate; 10 sites refactored across Predictor8/11/14/17. |
| L4 | `addLDD` / `mulLDD` (Smooth.lean) | **DONE** | `ldd_branch_via_maxRange (sf cx cy M hagree S R side_chain m' hm')` PURE template (in `Real213/Core/CutFnData.lean`) absorbs the inner `apply sf.prop; intro m'' k'' hm'' hk''; apply hagree; <chain>` block; 4 branches refactored across addLDD + mulLDD.  Parameterized helper instead of typeclass. |
...
| M  | `Raw.recAux` / `RawBy.recAux` pair | **DONE** | refactored to use Sub-2 helpers (Tree.canonical_slash_decompose / canonicalBy_slash_decompose). |
...
## Done summary
  ...
  · **§3 fully closed** — L1 (β + α, 4/4 siblings), C (CutSumOne 8/8).
  · **§4** — M, Pell-FSM (93 sites), ModArith, L3 (10 sites in
    Predictor8/11/14/17), **L4 DONE** (ldd_branch_via_maxRange
    template); L5 deferred (no consolidation possible); E/F still open.
  · **Cross-tier** — FLUX-1 (34 sites via
    CutMulOuterReduce + UnitBracketReduce/×2 + UnitBracketReduceSum),
    COH-1+2+3 (Pattern10 + InvolutionTemplate +
    LeibnizUniversalLift), **REAL-1+REAL-2 DONE**
    (BoolOrLadder.bool_or_ladder_iff template, 3 theorems refactored,
    ~140K Expr nodes retired), TH-1/TH-4 spec docs.

Net Lean deliverables:
...
(Σ-fold; additive).  L4 **DONE**.  L5 removed
(no yield).

Cross-tier closures: FLUX-1 (34 sites),
...
One research direction remains (not an abstraction item):
...
Ranked by mass × confidence / effort:
  ...
  3. **C** — broadest by sibling count (8); medium.
...
new abstraction pass to refresh candidate rankings.
