...
Generated by the scanners in `tools/` (see Regenerating below):
surface-citation counts, the Expr-level call graph, and the
type-signature-only hub list.

## Top E213-internal hubs (surface-citation level)
...
## Expr-level hubs (normalised by call graph)
...
drops and the internal-hub count rises.  Landed centralisations:
  ...
  · `Nat.max_comm` → `NatHelper.max_comm`.
  · `Int.mul_sub` / `Int.sub_mul` → `Meta.Int213.{mul_sub, sub_mul}`.
...
```
python3 tools/syntax_arg_scan.py    # surface citation counts
python3 tools/ast_callgraph_scan.py # Expr-level call graph
python3 tools/ast_typesig_scan.py   # type-signature only
```
...
## Cross-references
  ...
  · `theory/meta/methodology_patterns.md` "Adoption-gap detection
    via k-gram cascade scan" — the methodology that surfaces hub
