...
## Branch
`claude/p-adic-reciprocity-topics-qBPUW`.  **Five ∅-axiom closures** committed this session
...
| **reciprocity** | QR + supplements closed.  **Zolotarev** now has *both endpoints* PURE — the sign side `det_permMatrix`/`psign`, the count side `gauss_core`'s `μ`-parity — but the **equivalence of the two sign-readouts** (`psign[a·0,…,a·(p−1) mod p] = (a/p)`) is the genuinely hard residual: the clean homomorphism route (`a↦psign(π_a)`, squares in kernel) needs a **primitive root / `(p−1)`-cycle** for nontriviality at every `p` (no universal non-residue witness for `p≡1 mod 4`), and the repo has **no primitive-root infrastructure** — a multi-file undertaking, not a session seed.  Cubic/biquadratic reciprocity is very hard. |
