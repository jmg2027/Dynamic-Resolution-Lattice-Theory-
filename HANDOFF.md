...
**Dual-span scaffolding closed (2026-05-24)** in
`V33EnrichedParametricDualSpan.lean` (13 PURE):
  ...
  · inductive `InCupSpanPlusBoundary` (F₂-span of cup-products +
    coboundaries)
  · `codim_upper_bound_conditional` + `parametric_dual_span_capstone`:
    conditional on the per-layer joint-ψ-kernel ⊆ `InCupSpanPlusBoundary`
    hypothesis, the c ψ-discriminators SPAN the dual of
    `EnrichedFaceVal c / InCupSpanPlusBoundary`.

**Open piece**: the per-layer K_{3,3} cup-image span hypothesis
(joint ψ-kernel ⊆ cup-image-span + δ¹-image).  Reduces to a single
fact at one K_{3,3} layer: the 9-element face space modulo
(im δ¹ + cup-image) has dim 1, generated by `e_face_layer m`.
Layer-disjointness lifts this per-layer fact to the c-layer
