...
A–D + B are now **all closed ∅-axiom this arc**:
  ...
  - **CF universality — the det-one floor is universal** (`Real213/ContinuedFractionFloor`, 8/0): for ANY partial-quotient sequence `a`, the continued-fraction convergents `cfP/cfQ` (over Int) have cross-determinant `cf_det_sq`: W_n² = 1 — the universal Cassini engine `cf_det_step` (W_{n+1} = -W_n, the q=1 case via `cassini_one` over any CommRing213). So EVERY real, via its continued fraction, sits on the W=±1 det-one floor; `FibCassiniNat`(φ) is the all-1s instance. Rate engine added: `cfQn`/`cfQn_pos`/`cfQn_fib` (denominators as Nat, positive, Fibonacci-growing q_{n+2}>=q_{n+1}+q_n => gaps 1/(q_n q_{n+1}) shrink), `cfQ_eq_cast` (the Int cfQ is their cast). Framing (other branch): CF = the expansion engine -- distinction (floor) leaves a unit residue, that residue is the next operand (x|->1/(x-floor)), gapless because the unit W=+-1 is indivisible (cf_det_sq) + no exterior. Next: even-indexed convergents + CrossDetSmall -> free modulus (CF universal completion, generalizing CrossDetConstDenom-phi).
