...
1. Mellin transform gives π^{-s} Γ(s) ζ(**2s**). The “2s” comes from n².
1. Substituting s → s/2 yields the standard form with Γ(**s/2**).
1. The “2” in n² is the L² norm exponent: |z|² = zz̄.
1. Why is the norm quadratic? Because for any z ∈ K (normed division algebra), the subalgebra ℝ(z) ≅ ℂ, so z satisfies a degree-2 minimal polynomial t² − 2Re(z)t + |z|² = 0.
1. Therefore the norm exponent 2 = [ℂ:ℝ] = dim_ℝ(ℂ).
...
1. ℂ has dim_ℝ = 2 (unique extension atom, FTA)
1. 2 is the smallest additive atom (elementary)
1. ℂ⁵ = ℂ² ⊕ ℂ³ is the unique chiral decomposition (Paper 1)
1. The temporal sector has n_T = 2 = dim_ℝ(ℂ) (doubly irreducible, this theorem)
1. Phase equipartition: E[cos²θ] = 1/dim_ℝ(ℂ) = 1/2 (spherical symmetry)
1. CLT boundary: σ_stat = 1/2 (Kolmogorov three-series)
1. σ_stat = σ_geom only for K = ℂ (Two Boundaries Theorem)
