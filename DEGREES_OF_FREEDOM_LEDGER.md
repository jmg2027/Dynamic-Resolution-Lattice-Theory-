...
| **Jarlskog `J`** (θ_QCD prefactor) | `CPViolation` + `ThetaQCD` (`theta_QCD_num := 286` hardcoded) | *(self-audit, upgraded)* only the λ-power **structure** is derived; computing `J` from DRLT's own factors (λ=5/22, A=φ/2, δ=π/φ²) gives **8.18×10⁻⁵ vs observed 3.08×10⁻⁵ — ×2.66 over** (missing Wolfenstein apex `(ρ,η)`; `s₁₃=Aλ³` omits `√(ρ²+η²)≈0.39`). A `CPViolation.lean` comment had masked this with an arithmetic error ("within 10%"); corrected. θ_QCD (`PRE_REGISTRATION.md` P2) was inconsistent with the catalog value; an atomic **candidate** now closes most of it: `R_u=√(ρ²+η²)=c/d=2/5` (`JarlskogApex`) gives `|V_ub|`=0.00380 (0.5% vs obs) and `J`=3.27×10⁻⁵ (6% vs obs, was 166%). Candidate, not forced — the `c`-numerator awaits a forcing theorem. Frontier: `ckm_rho_eta_apex.md` |
