...
1. The propagator sum Σ 1/n^s with s = 2 (Theorem 4) produces ζ(2).
1. The oscillatory version Σ e^{iθ_n}/n^σ has convergence boundary σ = 1/2 = σ_stat.
1. If the arithmetic Dirichlet series Σ μ(n)/n^s inherits the phase statistics of U(ℂ) (Conjecture), then RH follows.
...
1. **Multiplicative structure.** The CLT argument uses iid phases. The Euler product ζ(s) = Π(1-p^{-s})^{-1} imposes multiplicative dependence. Does the coincidence σ_stat = σ_geom survive this dependence?
1. **Higher L-functions.** For Dirichlet L-functions L(s,χ), the GRH places zeros on Re(s) = 1/2. The characters χ(n) take values in roots of unity ⊂ U(ℂ). The Two Boundaries Theorem applies equally, suggesting GRH has the same structural origin as RH.
