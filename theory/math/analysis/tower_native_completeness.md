...
adjudication, closure of the finite-coordinate class, the generator, the residue tie,
the intensional-reduction split, the discriminant trichotomy, and the five-floor
unification.  The genuinely open questions are: whether every completing real admits a
*rate-carrying re-presentation* (the converse of the bridge; π via Wallis is the
obstruction, so this reframes the π frontier as an existential, not a property of π);
the **cross-presentation rung floor** — the rung over *all* presentations of a cut (the
rescaling sub-family is closed in `ScalingOrbit`; the cross-presentation invariant
subsumes the re-presentation question); and the standing **fast-π representation**
meeting the rate criterion (`holonomic_modulus.md` frontier).  Outside the tower-native
frame and not claimed: full num/den closure under `+`/`×` for arbitrary rate-carrying
presentations, and a fully generic ordinal-indexed `coord → cut` map.
