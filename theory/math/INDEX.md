...
- [`graded_residue_arithmetic.md`](graded_residue_arithmetic.md) — GRA (Graded Residue Arithmetic): unified theory. 7 axioms, 5 Readings (cohomology/operad/HoTT/graph/analysis), GRA Tower ↔ CD Tower duality, number system unification, dimensional proliferation fractal, Adelic decomposition. **CLOSED** (Marathon 16 — `lean/E213/Lib/Math/GRA/` umbrella, 8 files / 0 sorry).
- [`gra_book.md`](gra_book.md) — **GRA Book** (textbook treatment): Ch.0–8 + appendices. Formal Definition→Theorem→Proof flow. Gap 1–6 resolutions. Lean cross-reference table (Appendix B.1) covers the `GRA/` sub-tree.
