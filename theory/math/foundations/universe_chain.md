...
`Lens/Number/Nat213/Core.lean` defines `Nat213` as the subtype
`{ n : Nat // 1 ≤ n }` — the positive naturals (every Raw carries
at least one atom, so the count is `≥ 1`):

```
def Nat213 : Type := { n : Nat // 1 ≤ n }
```

The atomic 4-tuple `(NS, NT, d)` is recorded separately as a
constructor-count correspondence (see `AtomicityCorrespondence.lean`
below): `raw_constructor_count` (3 = NS), `nat213_constructor_count`
(2 = NT), `partition_sum` (NS + NT = d).
...
`Lib/Math/Algebra/Mobius213ModFive.lean` closes this explicitly:
`P_pow_5_eq_neg_I_mod_5` and `P_pow_10_eq_I_mod_5`.  Lucas / Pell-Fib
sit in `Lib/Math/Algebra/Mobius213/Px/`.
...
|---|---|---|
| `Nat213` type | `Lens/Number/Nat213/Core` | `{ n : Nat // 1 ≤ n }` subtype |
| `add` / `toNat_add` | `Lens/Number/Nat213/Core` | Addition + its toNat compatibility |
| `raw_constructor_count`, `nat213_constructor_count`, `partition_sum` | `Lens/Number/Nat213/AtomicityCorrespondence` | 3=NS, 2=NT, NS+NT=d |
...
| `P_pow_10_eq_I_mod_5` | `Lib/Math/Algebra/Mobius213ModFive` | P^10 ≡ I (mod 5) |
| Lucas / Pell-Fib | `Lib/Math/Algebra/Mobius213/Px/` | L_0 = 2, L_1 = 3, L_2 = 5, L_3 = 7 |
