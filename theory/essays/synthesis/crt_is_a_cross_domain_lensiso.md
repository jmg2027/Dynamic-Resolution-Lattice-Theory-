...
moduli must be positive and coprime; without coprimality only the meet direction holds (the
product reading is then strictly finer than `L_{mk}`).
...
## Cross-frame connections
  ...
  - **`the_substance_test.md`** "State of Line A" — this is the number-theoretic
    primacy-witness the marathon was assessing; here closed for all coprime moduli.
