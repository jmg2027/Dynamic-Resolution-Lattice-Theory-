...
compute** from its graded count: the least number of times the forward
difference annihilates the count.  No `∞` is ever written; an infinite-
dimensional object is one whose difference tower simply does not terminate.
...
Its **dimension is the annihilation depth of the forward difference**
`Δf(d) = f(d+1) − f(d)`.  Each difference drops the rung by one
(`MultSystem.diff_drops_rung`: `Δ(monoCount (k+1)) = monoCount k` shifted), so
`Δ^k` leaves the constant `1` (`diffIter_dim_const`) and `Δ^{k+1}` annihilates it
(`diffIter_dim_zero`).  The dimension `k+1` *is* that least depth — read off by
finite arithmetic, computable (`monoCount 3 = [1,3,6,10,…] → Δ²=[1,1,…] →
Δ³=[0,…] ⇒ dim 3`).
...
(`×`, all primes as axes) is not a thing with cardinality `ℵ₀` but precisely *the
count whose difference tower never terminates*, equivalently whose Hilbert series
has an essential singularity (the Euler product, `ζ`).

## Cross-frame connections

The same depth recurs as different invariants in neighbouring arcs.  The
finite-difference depth is the **additive** annihilation depth (a polynomial /
C-finite sequence with all roots `1`); its multiplicative twin is the
**Casoratian / Wronskian** depth of a holonomic sequence (the depth-order
duality of the sequence-depth arc) — `Δ`/`Σ` and the multiplicative
`conv`/Casoratian are the same depth operator at two resolutions.  In cohomology
