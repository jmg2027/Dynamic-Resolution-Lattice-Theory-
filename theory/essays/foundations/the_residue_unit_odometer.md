...
- `lean/E213/Theory/Raw/OdometerValue.lean` (11 PURE, 0 DIRTY) — the profinite value `bval` and
  the carry-explicit `+1 mod 2ᵏ` identity (`bval_odo`) + fixed-point-freeness.
...
fixes no stream — the act of pointing always changes something (the `j = 1` case of the
`ℤ`-action's freeness).  The arithmetic here is pure `Nat` (no `omega`, which is propext-tainted).
...
## Open frontier
  ...
  - **Freeness of the `ℤ`-action** — the `k = 1` case is *closed* (`odo_no_fixpoint`: the `+1`
    flips the low bit, so `odo f ≠ f`).  The general `odoʲ f = f → j = 0` needs iterating
    `bval_odo` (the value advances by `j mod 2ᵏ`) — the concrete remaining path.
