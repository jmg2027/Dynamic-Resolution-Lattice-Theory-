...
- `lean/E213/Theory/Raw/Odometer.lean` (38 PURE, 0 DIRTY) — the binary `+1` odometer on the
...
## Open frontier
  ...
  - **Freeness of the `ℤ`-action** — `odoᵏ f = f → k = 0` (no nonzero period).  The `k = 1` case
    is immediate (`odo` flips the low bit, so `odo f ≠ f`); the general case needs the iterate and a
    value-mod-`2ᵏ` argument (open here).
