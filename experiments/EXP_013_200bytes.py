...
class Exp(Experiment):
    ...
    def run(self):
        ...
        self.log(f"  N_gen = {drlt.generation_count()}")
        self.check("eta_B ~ 6e-10", abs(drlt.baryon_asymmetry()/6.1e-10 - 1) < 0.01)
        self.check("Omega_L ~ 0.685", abs(drlt.dark_energy_fraction() - 0.685) < 0.005)
