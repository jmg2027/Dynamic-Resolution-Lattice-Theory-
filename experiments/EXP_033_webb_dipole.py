...
class Exp(Experiment):
    ...
    def run(self):
        ...
        self.log(f"  1/alpha_em = {inv_a:.3f}, alpha_GUT = {a_gut:.6f}, Xi = {xi:.6e}")
        ...
        self.check("Da_em amplitude consistent with Xi mediation", da_amp > 1e-8)

        # alpha_GUT spatially invariant
        inv_a_gut = drlt.inv_alpha_gut()
        self.log(f"  1/alpha_GUT = {inv_a_gut:.4f} (spatially invariant)")
        self.check("alpha_GUT is structural constant", abs(inv_a_gut - 25*np.pi**2/6) < 1e-10)
