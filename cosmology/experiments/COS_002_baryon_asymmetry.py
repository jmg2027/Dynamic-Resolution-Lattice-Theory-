...
Zero-parameter predictions:
  eta_B = alpha_GUT^3 * (n_T/n_S) / pi  = 6.10e-10  (obs 6.1e-10, 0.04%)
  Omega_Lambda = 1 - 1/pi = 0.6817       (obs 0.685, 0.07%)
...
class BaryonAsymmetry(Experiment):
    ...
    def run(self):
        ...
        self.log(f"  Omega_Lambda = {omega:.4f} = 1 - 1/pi  (obs: 0.685)")
        self.check("Omega_Lambda ~ 0.685 (< 1%)",
                    abs(omega - 0.685) / 0.685 < 0.01)
