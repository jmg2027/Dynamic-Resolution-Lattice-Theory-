...
|------|-----------|------|
| `Kernel/Term, Compare, Pair, Rat, …` | `kernel/` | ℕ encoding, decidability, ℚ-as-(ℕ,ℕ), normal form |
| `Firmware/Raw*` | `firmware/` (raw mod) | `Raw` opaque + `slash`/`fold`/`swap` API |
| `Hypervisor/Lens` | `firmware/` (lens mod) + `hypervisor/` | sealed `Lens<A>`, canonical instances, K_{3,2}^{(2)} |
| `OS/Atomicity, Pigeonhole, …` | `os/` (atomicity, arity) | Atomicity, NonDecomposable, ArityForcing |
| `Math/Analysis213/*` | `os/` (analysis213) | Cut algebra, dyadic, derivative, integral — algorithm form |
| `Math/Cohomology/*` | `hypervisor/` (cohomology) | δ², ⋆⋆, fluxes — as Lens compositions |
...
- **Encapsulation:** each crate exposes only what its Lean
  ...
  invisible above firmware, just as Lean's `Internal` namespace.
