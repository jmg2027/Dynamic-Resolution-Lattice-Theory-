...
//! proximity to a target (e.g. the α_em structural gap 5.4×10⁻⁴).
//!
//! Pure-ℚ = derivable from {NS, NT, d, c} + chiral_dim + b1_bipartite
//! + α_GUT bracket via ×, ÷.  No floats.  No ad-hoc integers.
...
/// |a − b| as ℚ pair (num, den).  Use `lt_q` to compare two of these.
...
/// ℚ-pair `<`: `p.0/p.1 < q.0/q.1` ⇔ `p.0·q.1 < q.0·p.1`.
pub fn lt_q(p: &Q, q: &Q) -> bool { &p.0 * &q.1 < &q.0 * &p.1 }

/// Decimal approx (ℕ-only).
...
pub fn candidates(n: u64) -> Vec<(String, Q)> {
    ...
    // α_GUT² · simple structural multipliers
    for &k in &[1u64, 2, 3, 5, 6, 9, 10] {
        v.push((format!("α_GUT²/{}", k),
            (asq.0.clone(), &asq.1 * nat(k))));
        v.push((format!("{}·α_GUT²", k),
            (&asq.0 * nat(k), asq.1.clone())));
    }
    v
}
