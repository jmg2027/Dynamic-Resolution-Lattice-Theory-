...
Our universe is not exact rank 1 (which would have $\hbar = 0$ exactly). It occupies the effective rank $\sim 5$ band, where residual eigenvalues are small but nonzero. A perfectly aligned vacuum ($\psi_i \propto \psi_0$ for all $i$) would have $\det G_h = 0$, hence $\hbar = 0$ and $E = 0$; the equation of state $w = p/\rho$ is undefined there ($\rho = 0$). Our universe never reaches this state:
\begin{equation}
