...
However, this formula gives $4R_y = 54.4\;\text{eV}$ (the He$^+$ value) because $\det(G_\text{ABB}) \approx \det(G_\text{AAB})$ when $B_1 \perp B_2$ (EXP-067/068).  The correct result requires the Binet--Cauchy channel structure.
...
\end{equation}
This is an algebraic identity of the 20-hinge decomposition (EXP-068, verified to $10^{-4}$).  The $O(\varepsilon^4)$ corrections contribute $<2\%$ of the needed correction and go in the wrong direction.

\textbf{Step 2.  BBB hinge channel weight.}
The hinge $\{B_1, B_2, X\}$ exists on $\partial(\Delta^5)$ only when both electrons couple.  Its Binet--Cauchy decomposition is $100\%$ in the $k=2$ (STT) channel (EXP-068 verified: $k=0$ and $k=1$ contributions are $< 10^{-20}$).  The $c$-weight of the $k=2$ channel is $c^2 = 4$.
