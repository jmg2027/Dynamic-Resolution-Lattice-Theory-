...
Perturbative $S_3$ breaking enters at order $\alpha_\text{GUT}$ per vertex pair, with $c \cdot n_S = 6$ independent channels contributing multiplicatively.  The variational principle $\delta S/\delta\psi = 0$ correlates $\theta_\text{bare}$ with $\arg\det(Y_u Y_d)$, enforcing cancellation up to order $\alpha^5$.  The residual phase at sixth order is quantised in units of $2\pi/(d^2-1)$, the minimal SU(5) phase quantum (the same quantum appearing in $\delta_\text{PMNS}$):
\begin{equation}
\boxed{\;\theta_\text{QCD} = \alpha_\text{GUT}^{c\cdot n_S}\;\sin\!\frac{2\pi}{d^2-1} = \alpha_\text{GUT}^6\;\sin\frac{\pi}{12} \approx 5.4 \times 10^{-11}.\;}
\end{equation}
Well within the experimental bound $|\theta_\text{QCD}| < 1.8 \times 10^{-10}$ (nEDM, 90\% CL).  \textbf{No axion is required.} The strong CP problem is solved by the permutation symmetry of spatial vertices --- a geometric fact, not a dynamical mechanism.  The prediction $\theta \neq 0$ distinguishes DRLT from axion models ($\theta \to 0$) and is testable by next-generation nEDM experiments.
...
19 & $\theta_\text{QCD}$ & $5.4\!\times\! 10^{-11}$ & $< 1.8\!\times\! 10^{-10}$ & $\alpha_\text{GUT}^6\sin(\pi/12)$ \\
\hline
