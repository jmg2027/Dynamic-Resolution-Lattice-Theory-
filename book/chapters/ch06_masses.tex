...
R = \rho \times L,
\end{equation}
where $\rho = n_A/n_B = 1/\det(G_\mathrm{ABB})$ is the impedance per hop (from the ABB hinge determinant), and $L = 1/\alpha$ is the correlation length of the lattice propagator with nearest-neighbor coupling amplitude $\sqrt{W_{AB}} \propto \alpha$.  The lattice Green's function $G(n) \sim e^{-n/\xi}$ with $\xi \sim 1/\alpha$ is a standard result; thus $R = (n_A/n_B) \times (1/\alpha) = n_A/(n_B\alpha)$.
