...
The cost of crossing one generation boundary through an AAAB face is set by the spatial hinge count $n_A$, the temporal efficiency $n_B$, and the electromagnetic coupling $\alpha$:
\begin{equation}
\label{eq:mu_e}
\boxed{\frac{m_\mu}{m_e} = \frac{n_A}{n_B\,\alpha}\left(1 + \frac{\alpha_\mathrm{GUT}}{n_A + 1}\right) = \frac{3}{2\alpha}\left(1 + \frac{\alpha_\mathrm{GUT}}{4}\right) = 206.80.}
\end{equation}
Observed: $206.768$ ($0.02\%$).  The ingredients $W_{AB} = \alpha^2/(n_A d)$ and $|\langle B|B_3\rangle|^2 = 1/c = 1/n_B$ have been verified numerically to 6 and 4 digits respectively (Appendix~\ref{app:verification}).  A complete algebraic derivation connecting these ingredients to the impedance formula is in progress; the mathematical structure suggests that a closed-form proof is achievable.
