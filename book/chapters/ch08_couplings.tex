...
\end{equation}
The temporal dimension count $n_T = 2$ emerges as the ratio of non-alternating to alternating propagator sums.  This is not a numerical coincidence: the identity $\zeta(s)/\eta(s) = 2^s/(2^s - 2)$ gives $\zeta(2)/\eta(2) = 4/2 = 2$ for $s = 2$ (= $n_A - 1 = $ spatial propagator exponent), and the Dirichlet eta arises \emph{because} the vacuum holonomy is $\pi$.
\end{remark}
