...
\title{The Discrete Geometric Origin of the Yang--Mills Mass Gap\\
and the No-Go Theorem for Wightman Axioms}
\author{Mingu Jeong\\Independent Researcher}
\date{April 2026}
\maketitle

\noindent
\textbf{MSC 2020:} 81T13 (primary), 83C27, 57Q15, 81V05\\
\textbf{Keywords:} Yang--Mills mass gap, Regge calculus, simplicial
complex, color confinement, Wightman axioms, discrete spacetime

\begin{abstract}
We prove that the Yang--Mills mass gap is a theorem of discrete geometry.
In the Dynamic Resolution Lattice Theory (DRLT) framework, spacetime is a
finite simplicial complex on $\mathbb{C}P^4$, and the strong interaction
resides on AAA hinges --- triangles whose three vertices span the spatial
sector $\mathbb{C}^3 \subset \mathbb{C}^5$. The proof requires three facts:
(i)~$\det(G_{\mathrm{AAA}}) > 0$ because three spanning vectors in
$\mathbb{C}^3$ are linearly independent;
(ii)~the deficit angle $\delta_{\mathrm{AAA}} = \pi/2 > 0$, computed
exactly on $\partial\Delta^5$ from the orthogonality of the $(3,2)$ split;
(iii)~confinement ($N_{\mathrm{eff}} = 1$) prevents the gap from closing.
Therefore $\Delta = \sqrt{\det} \times \delta > 0$.
We further prove a no-go theorem: in the continuum limit required by the
Wightman axioms, $\det \to 0$ forces $\Delta = 0$. The mass gap and
continuous spacetime are mutually exclusive. The Clay Millennium Problem,
as formulated on $\mathbb{R}^4$, admits no solution with $\Delta > 0$
--- not because the proof is too hard, but because the premise is wrong.
\end{abstract}
...
\cite{DRLT, DRLT-rep}, spacetime is not a continuum approximated by a
lattice; spacetime \emph{is} a finite simplicial complex on
$\mathbb{C}P^4$. The mass gap is then a theorem of discrete algebraic
geometry, following from three facts:

\begin{enumerate}
\item The strong force lives on AAA hinges --- triangles with all three
  vertices in the spatial sector $\mathbb{C}^3$.
...
$\mathbb{C}^3$ rank saturation. Section~\ref{sec:massgap} proves the
...
$\mathbb{R}$ are $\{\mathbb{R}, \mathbb{C}, \mathbb{H}\}$. $\mathbb{R}$
fails R2 ($\{+1,-1\}$ is discrete). $\mathbb{H}$ fails R3
(non-commutative). $\mathbb{C}$ satisfies all four, with
$\pi_1(S^1) = \mathbb{Z}$.

Each point $i$ carries a state $\psi_i \in \mathbb{C}^d$, and the
pairwise relation is the Gram matrix:
\begin{equation}
G_{ij} = \langle \psi_i | \psi_j \rangle, \qquad
G \in M_{N \times N}(\mathbb{C}), \qquad
\mathrm{Tr}(G) = N.
\end{equation}

\subsection{The $(3,2)$ split and gauge group}

The unique decomposition of $\mathbb{C}^d$ into atomic blocks supporting
chiral fermions, CP violation, and charge quantization is
$(n_S, n_T) = (3, 2)$, yielding $d = 5$ \cite{DRLT-rep}
(cf.\ \cite{Georgi-Glashow} for the original SU(5) GUT). This gives:

\begin{itemize}
\item Base space: $\mathbb{C}P^4$ (projectivization of
  $\mathbb{C}^5 \setminus \{0\}$).
\item Gauge group: $\mathrm{SU}(3) \times \mathrm{SU}(2) \times
  \mathrm{U}(1)$.
\item Spatial sector: $\mathbb{C}^3$ (indices $a = 0, 1, 2$).
\item Temporal sector: $\mathbb{C}^2$ (indices $\alpha = 3, 4$),
  orthogonal to $\mathbb{C}^3$.
\end{itemize}
...
\subsection{Simplicial complex and Regge calculus}

The $N$ vertices of $G$ induce a finite simplicial complex $K$ on
$\mathbb{C}P^4$. Each 4-simplex $\sigma$ has 5 vertices; each 2-face
...
%=====================================================================
\section{Confinement as $\mathbb{C}^3$ Rank Saturation}
...
$\mathbb{C}^3$. A second hop requires a \emph{second independent} AAA
...
$\mathbb{C}^3$ has exactly three dimensions.
\end{remark}
...
\label{lem:det}
For any three vertices $\{a, b, c\}$ spanning $\mathbb{C}^3$, the
...
\begin{proof}
The three spatial state vectors $\psi_a, \psi_b, \psi_c \in \mathbb{C}^3$
...
\begin{equation}
\boxed{\delta_{\mathrm{AAA}} = \frac{\pi}{2}.}
\end{equation}
\end{lemma}
...
within each tetrahedron. Since $\mathbb{C}^3 \perp \mathbb{C}^2$ in
...
\begin{remark}
The deficit angle $\delta_{\mathrm{AAA}} = \pi/2 > 0$ means the AAA
hinge carries strictly positive curvature. This is a consequence of
$\mathbb{C}^3 \perp \mathbb{C}^2$: the spatial and temporal sectors
...
over $\mathbb{C}P^4$ has a strictly positive mass gap $\Delta > 0$.
\end{theorem}
...
By Lemma~\ref{lem:delta}: $\delta_{\mathrm{AAA}} = \pi/2 > 0$.
...
\frac{\pi}{2} > 0.}
\end{equation}
For the orthonormal configuration, $\Delta = \pi/2$ in natural
(Planck) units.
\end{proof}

\begin{corollary}
The lightest glueball mass is $m_G = \Delta = \pi/2$ in Planck units
...
\item[(i)] $\det(G_{\mathrm{AAA}}) > 0$: three spanning vectors in
  $\mathbb{C}^3$ are linearly independent.
\item[(ii)] $\delta_{\mathrm{AAA}} = \pi/2 > 0$:
  $\mathbb{C}^3 \perp \mathbb{C}^2$ forces positive curvature at the
  AAA hinge.
...
%=====================================================================
\section{No-Go Theorem for Continuous Spacetime}
\label{sec:nogo}
%=====================================================================

\begin{definition}[Continuum limit]
\label{def:continuum}
The continuous spacetime $\mathcal{M}_{\mathrm{cont}} = \mathbb{R}^4$ is
...
spacetime ($\mathbb{R}^4$) satisfying the Wightman axioms are mutually
exclusive.
\end{theorem}
...
$\mathbb{R}^4$, which implies the continuum limit
...
\begin{corollary}
The Clay Millennium Problem, as formulated on $\mathbb{R}^4$ with the
...
$\alpha_{\mathrm{GUT}} = 6/(25\pi^2)$ is a mathematical constant
derived from the geometry of $\mathbb{C}^5$ \cite{DRLT-rep}. The hinge
...
\subsection{Lattice QCD versus DRLT}
...
Confinement & from flux tubes & from $\dim(\mathbb{C}^3) = 3$ \\
\hline
\end{tabular}
\end{center}
...
\subsection{The Millennium Problem reconsidered}

The Clay problem asks: ``Prove $\Delta > 0$ on $\mathbb{R}^4$.''
Theorem~\ref{thm:nogo} shows that $\Delta > 0$ and $\mathbb{R}^4$ are
mutually exclusive. This does not mean the mass gap is false --- it
means the premise ($\mathbb{R}^4$) is wrong. The correct formulation:

\begin{quote}
\emph{The Yang--Mills mass gap exists on the natural simplicial complex
$K$ over $\mathbb{C}P^4$, with $\Delta = \sqrt{\det(G_{\mathrm{AAA}})}
\times \pi/2$, and it cannot exist on $\mathbb{R}^4$.}
\end{quote}
...
\item $\det(G|_{\mathrm{AAA}}) = 1.000000 \pm 0$ for orthogonal
  $\mathbb{C}^3$ basis (EXP-043b).
\item Dihedral angle $\theta_{\mathrm{AAA}} = 90.00^\circ$ exactly
  (EXP-042a).
...
\subsection{Why the problem was hard}
...
($\mathbb{R}^4$) destroys the very geometric structure that creates the
...
The resolution is not a more clever analytic technique on $\mathbb{R}^4$.
It is the recognition that spacetime is a simplicial complex on
$\mathbb{C}P^4$, that the strong force lives on AAA hinges, and that
...
\bibitem{DRLT-rep} M.~Jeong, ``The Representation-Theoretic Uniqueness
  of $(2,3)$: Why Physics is Always
  $\mathrm{SU}(3)\times\mathrm{SU}(2)\times\mathrm{U}(1)$,''
  preprint (2026).

\bibitem{Clay} A.~Jaffe and E.~Witten, ``Quantum Yang--Mills Theory,''
  in \emph{The Millennium Prize Problems} (Clay Mathematics Institute,
  Cambridge, MA, 2006), pp.~129--152.

\bibitem{Regge} T.~Regge, ``General relativity without coordinates,''
  \emph{Nuovo Cimento} \textbf{19}, 558--571 (1961).

\bibitem{Wilson} K.~G.~Wilson, ``Confinement of quarks,''
  \emph{Phys.\ Rev.\ D} \textbf{10}, 2445--2459 (1974).

\bibitem{Creutz} M.~Creutz, ``Monte Carlo study of quantized SU(2)
  gauge theory,'' \emph{Phys.\ Rev.\ D} \textbf{21}, 2308--2315 (1980).

\bibitem{Wightman} A.~S.~Wightman, ``Quantum Field Theory in Terms of
  Vacuum Expectation Values,'' \emph{Phys.\ Rev.}\ \textbf{101},
  860--866 (1956).

\bibitem{OS} K.~Osterwalder and R.~Schrader, ``Axioms for Euclidean
  Green's Functions,'' \emph{Commun.\ Math.\ Phys.}\ \textbf{31},
  83--112 (1973).
...
\bibitem{tHooft} G.~'t~Hooft, ``A Planar Diagram Theory for Strong
  Interactions,'' \emph{Nucl.\ Phys.\ B} \textbf{72}, 461--473 (1974).

\bibitem{Ambrose-Singer} W.~Ambrose and I.~M.~Singer,
  ``A Theorem on Holonomy,'' \emph{Trans.\ Amer.\ Math.\ Soc.}\
  \textbf{75}, 428--443 (1953).
