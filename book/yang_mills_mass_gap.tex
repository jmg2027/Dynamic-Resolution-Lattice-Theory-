...
(ii)~the deficit angle $\delta_{\mathrm{AAA}} = \pi > 0$, computed
from the variational principle $\delta S/\delta\psi = 0$ on $\partial\Delta^5$;
...
\times \pi$, and it cannot exist on $\mathbb{R}^4$.}
\end{quote}
