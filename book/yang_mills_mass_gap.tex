...
\noindent
Lattice QCD provides strong numerical evidence: Monte Carlo simulations
on finite lattices consistently produce a mass gap, and the result
survives extrapolation to the continuum limit. But numerical computation
is not mathematical proof. Decades of effort have produced no
rigorous proof within the continuum framework.
...
$(n_S, n_T) = (3, 2)$, yielding $d = 5$ \cite{DRLT-rep}. This gives:
...
The Ambrose--Singer theorem guarantees that trivial holonomies imply
a flat connection --- there is no Yang--Mills dynamics left.
\end{remark}
...
\bibitem{DRLT-rep} M.~Jeong, ``The Representation-Theoretic Uniqueness
  ...
  $\mathrm{SU}(3)\times\mathrm{SU}(2)\times\mathrm{U}(1)$'' (2026).
