...
By Frobenius, the finite-dimensional associative division algebras over $\mathbb{R}$ are $\{\mathbb{R}, \mathbb{C}, \mathbb{H}\}$ (octonions excluded by non-associativity). $\mathbb{R}$ fails R2 ($\{+1,-1\}$ is discrete). $\mathbb{H}$ fails R3 (non-commutative, determinant ambiguous). $\mathbb{C}$ satisfies all four, with $\pi_1(S^1) = \mathbb{Z}$ (R4).
...
The $\sim$2.4\% ghost corrections throughout DRLT --- in the proton mass, water bond angle, baryon asymmetry, and coupling constants --- are now identified as contributions from $\lambda_6, \ldots, \lambda_{d_\text{indep}}$: spectrally indistinguishable from $\lambda_1, \ldots, \lambda_5$ but representation-theoretically trivial.

The ghost sum rule $\sum_i \delta(1/\alpha_i) = 0$ follows from $\text{Tr}(G) = N$, which holds regardless of how many dimensions are trivial. The same $\alpha_\text{GUT}$ correction appears in:
...
\bigskip
\noindent\textbf{AI Disclosure.} Developed with assistance from Claude (Anthropic).
