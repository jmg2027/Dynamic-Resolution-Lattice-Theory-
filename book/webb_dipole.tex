...
\title{The Fine Structure Constant is Not a Constant:\\$\det(G_h)$ Redistribution and the Webb Dipole}
\author{Mingu Jeong\\Independent Researcher}
\date{\today}
\maketitle

\begin{abstract}
We show that the observed spatial variation of the fine structure constant $\alpha_\text{em}$ (the Webb dipole, $\Delta\alpha/\alpha \sim 10^{-5}$) is a natural consequence of the DRLT $\det(G_h)$ redistribution mechanism. The fundamental coupling $\alpha_\text{GUT} = 6/(25\pi^2)$ is a true constant, protected by trace conservation ($\operatorname{Tr}(G) = N$). However, the individual couplings $\alpha_3, \alpha_2, \alpha_1$ receive $\det(G_h)$ geometric contributions $\Delta_i$ that sum to zero but whose \emph{spatial distribution} can vary. A variation $\Delta\varepsilon_0/\varepsilon_0 \sim 0.17\%$ --- comparable to the CMB dipole ($v/c = 0.12\%$) --- reproduces the observed $\Delta\alpha/\alpha \sim 10^{-5}$. The theory makes a falsifiable prediction: $\alpha_s$ must vary in the \emph{opposite direction} to $\alpha_\text{em}$, maintaining $\sum\Delta_i = 0$ at every spatial location.
\end{abstract}
...
We propose a resolution from Dynamic Resolution Lattice Theory (DRLT) \cite{DRLT}: \textbf{$\alpha_\text{em}$ is not a fundamental constant. It is a local manifestation of $\det(G_h)$ redistribution.} The true constant is $\alpha_\text{GUT} = 6/(25\pi^2)$, derived from lattice geometry. The measured $\alpha_\text{em}$ varies because the geometric contributions $\Delta_i$ --- $\det(G_h)$ weights from the rank projection --- are spatially non-uniform. Both sides of the Webb debate are correct: $\alpha_\text{em}$ varies (Webb is right), but the underlying physics is universal ($\alpha_\text{GUT}$ is constant).

%=====================================================================
\section{Trace Conservation (Review)}
%=====================================================================
...
%=====================================================================
\section{Spatial Variation of $\det(G_h)$ Contributions}
%=====================================================================
...
\end{equation}
The total coupling $1/\alpha_\text{GUT} = 25\pi^2/6$ is spatially invariant. Only the distribution of geometric weight among sectors varies.
\end{theorem}
...
An earlier version of this analysis predicted $\Delta\mu/\mu$ would anti-correlate with $\Delta\alpha_\text{em}/\alpha_\text{em}$ through the chain $\alpha_s \to \Lambda_\text{QCD} \to m_p$. However, DRLT's own results (Chapter 6) show this chain is broken: $\Lambda_\text{QCD} = v_H/\sqrt{c} \times \alpha_\text{GUT}^2 \times n_A$ depends on $\alpha_\text{GUT}$ (trace-invariant), not on $\alpha_s$ (which varies with $\varepsilon_0$). Similarly, $m_e$ depends on the lattice constant $\delta_\text{EW} = 1 - S(2)/S(\infty)$. Therefore $\mu \approx \text{const}$ across the sky: $\Delta\mu/\mu \approx 0$. Published H$_2$ quasar data ($\sim 10$ systems, all within $1$--$2\sigma$ of zero) are consistent with this prediction.
\end{remark}
...
The Webb dipole is not a systematic error. It is not evidence for new fields or extra dimensions. It is the spatial non-uniformity of $\det(G_h)$ redistribution in $\mathbb{C}^5$, visible because $\alpha_\text{em}$ is a composite of individual sector couplings whose geometric weights vary spatially.

The 25-year debate between ``$\alpha$ varies'' and ``$\alpha$ is constant'' is resolved: both are right, about different quantities. $\alpha_\text{em}$ varies. $\alpha_\text{GUT}$ is constant. The difference is the $\det(G_h)$ redistribution among sectors.
