...
\begin{theorem}[Swap Automorphism: algebraic content]
\label{thm:swap}
Let $\CC^d = \CC^a \oplus \CC^a$ with $a \geq 2$.  The gauge group $G = \text{SU}(a) \times \text{SU}(a) \times \text{U}(1)$ admits an outer $\mathbb{Z}_2$ automorphism $\sigma$; fermion irrep pairs $(R_1, R_2)$ that are $\sigma$-fixed satisfy $R_1 \cong R_2$ (vector-like).  Vector-like representations admit bare masses and carry no chirality or CP asymmetry.
\end{theorem}
...
\textbf{Step 2: Representation-theoretic content (Case (a)).}
Assume the $\sigma$-invariant branch: $\sigma|\Omega\rangle = |\Omega\rangle$, and all physical observables are $\sigma$-fixed.  A fermion representation is a pair $(R_1, R_2)$ of irreps of $\mathrm{SU}(a)_1 \times \mathrm{SU}(a)_2$, on which $\sigma$ acts by $(R_1, R_2) \mapsto (R_2, R_1)$.  The pair is $\sigma$-fixed iff $R_1 \cong R_2$ (the left-handed and right-handed content agree up to isomorphism): the representation is \emph{vector-like}.  A vector-like rep admits a bare mass term and produces no chirality, no CP violation, and no Standard-Model-like force hierarchy.

This rep-theoretic statement (``$\sigma$-invariant iff vector-like'') is formalized and machine-verified in \texttt{PmfRh/SwapAnnihilation.lean} (theorem \texttt{sigma\_invariant\_iff\_vector\_like}).
\end{proof}

\begin{remark}[Case (b): the broken branch is a physics argument]
The alternative case $\sigma|\Omega\rangle \neq |\Omega\rangle$ is a spontaneous $\mathbb{Z}_2$ breaking: domain walls form between $|\Omega\rangle$ and $\sigma|\Omega\rangle$, and the ensemble-averaged physics is again $\sigma$-symmetric (standard QFT argument; see e.g.\ \cite{Kibble1976} for the domain-wall restoration mechanism).  This is a field-theoretic rather than purely algebraic conclusion and is listed separately from the proof above for clarity: the mathematical content of Theorem~\ref{thm:swap} is case (a), and case (b) is a physics consistency check that the broken branch cannot rescue chiral structure.
\end{remark>
