...
protected def allBelow : Nat → (Nat → Bool) → Bool
  | 0,     _ => true
  | n+1,   p => (Decide.allBelow n p) && p n

/-- Bool version of ∃ x < n, p x. -/
protected def existsBelow : Nat → (Nat → Bool) → Bool
  | 0,     _ => false
  | n+1,   p => (Decide.existsBelow n p) || p n
