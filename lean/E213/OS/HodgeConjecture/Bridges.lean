import E213.OS.HodgeConjecture.Bridges.BeilinsonLichtenbaum
import E213.OS.HodgeConjecture.Bridges.BlochBeilinson
import E213.OS.HodgeConjecture.Bridges.ChernCharacter
import E213.OS.HodgeConjecture.Bridges.HodgeTate
import E213.OS.HodgeConjecture.Bridges.MumfordTate
import E213.OS.HodgeConjecture.Bridges.Tate

/-! Spec-as-code entry point for `E213.OS.HodgeConjecture.Bridges`.

  Top-level OS-tier motivic-cohomology bridges — the
  classical-side counterparts of the Math/Cohomology/HodgeConjecture
  programme, expressed as cross-references to the standard
  algebraic-geometry literature.

  ## Files

    * `BeilinsonLichtenbaum`  — Beilinson–Lichtenbaum motivic
                                conjecture
    * `BlochBeilinson`        — Bloch–Beilinson filtration
    * `ChernCharacter`        — Chern character bridge
    * `HodgeTate`             — Hodge–Tate decomposition
    * `MumfordTate`           — Mumford–Tate group
    * `Tate`                  — Tate conjecture
-/
