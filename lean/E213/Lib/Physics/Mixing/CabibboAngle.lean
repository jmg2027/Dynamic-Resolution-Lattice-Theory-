...
DRLT formula:
    sin θ_C (bare) = D / (D² - D + NT) = 5 / (25 - 5 + 2) = 5/22

This is the cleanest possible 0-param DRLT prediction:
**single rational from {D, NT} integers**.  No ζ(2), no
...
/-- sin θ_C bare value: D / (D² - D + NT) as `(num, den)`.
    The order-2/signature factor on the denominator is `NT = 2`. -/
...
/-- ★ Cabibbo angle master — sin θ_C = 5/22 bare DRLT rational.
  ...
  (D² − D + NT = 22), DRLT overshoot 0.34% relative to observed
