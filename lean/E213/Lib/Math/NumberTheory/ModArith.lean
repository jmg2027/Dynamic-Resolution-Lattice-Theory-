...
import E213.Lib.Math.NumberTheory.ModArith.LensLcmMeet
...
import E213.Lib.Math.NumberTheory.FibZSums
...
import E213.Lib.Math.NumberTheory.EisensteinRepresentation
...
import E213.Lib.Math.NumberTheory.FibonacciApparition
...
/-! Spec-as-code entry point for `E213.Lib.Math.NumberTheory.ModArith`.
  ...
  ## Lens-CRT
    ...
    * `LensCRT`      — Chinese remainder theorem as Lens
                       composition
    * `LensLcmMeet`  — the Lens-lattice meet is the lcm-modulus:
                       `L_{lcm(m,k)} ≈ prodLens(L_m,L_k)` for all
                       positive `m,k` (CRT is the coprime case)
