...
namespace E213.Lib.Math.NumberTheory.DyadicFSM.BitFSM.Converse
...
/-- The cyclic FSM run at step k is ⟨k % p, _⟩.  STRICT ∅-AXIOM. -/
theorem bitFSMOfPure_run (bs : Nat → Bool) (p : Nat) (hp : 0 < p) :
    ∀ k, (bitFSMOfPure bs p hp).run k = ⟨k % p, Nat.mod_lt _ hp⟩ := by
  intro k
  induction k with
  | zero => apply Fin.ext; show 0 = 0 % p; rw [E213.Meta.Nat.AddMod213.zero_mod]
  | succ k' ih =>
    ...
    apply Fin.ext
    show (k' % p + 1) % p = (k' + 1) % p
    exact E213.Meta.Nat.AddMod213.mod_add_mod hp k' 1
...
theorem bitFSMOfPure_correct (bs : Nat → Bool) (p : Nat) (hp : 0 < p)
  ...
  have h1 : k % p + (k / p) * p = k := by
    have h0 : p * (k / p) + k % p = k := E213.Meta.Nat.AddMod213.div_add_mod k p
...
end E213.Lib.Math.NumberTheory.DyadicFSM.BitFSM.Converse
