...
| `ChainAntichain.lean` | **Mirsky's theorem on `2^[n]` (chain/antichain duality), CLOSED**: `chain_card_inj` (the *chain* SEPARATE — comparable equal-size members coincide, dual of the antichain SEPARATE), `IsChain`, ★ `chain_length_le` (height ≤ `n+1`, size map ↪ `idxList(n+1)`), `initSeg`/`canonChain` + ★ `canonChain_max` (the height is **achieved**: the canonical chain `∅⊂{0}⊂…⊂[n]` is a nodup chain of exactly `n+1`), `chain_layer_cap` (chain meets each layer ≤ once), `mem_own_layer` (the layers cover), ★★ `mirsky_boolean` — longest chain = `n+1` = #layers (the `n+1` size-layers are the minimum antichain partition).  **Dilworth lower bound**: `memBL`/`findChain` + ★★ `dilworth_lower` (any chain cover ≥ `C(n,⌊n/2⌋)` = max antichain; the chain-cover dual).  `mem_idxList_of_lt`.  **SCD upper bound** (§6–11): `scd`/`extendC`/`raiseC`, `scd_isChain` + ★★ `scd_chain_cover` (chain + cover), `scd_nonempty`; the **symmetric-level invariant** `consec`/`extendC_sym`/`raiseC_sym` + length helpers + ★★ `scd_sym` (every chain's `cardB` run is `[k,…,n−k]`, `2k+|C|=n+1`); `sym_span`/`mem_consec` + `scd_has_middle`/`scd_middle_unique` (each chain meets `⌊n/2⌋` exactly once) + `scd_lower` (`C(n,⌊n/2⌋) ≤ |scd n|`).  (Only partition-disjointness → `|scd n| ≤ C(n,⌊n/2⌋)` remains.) | ∅-axiom |
