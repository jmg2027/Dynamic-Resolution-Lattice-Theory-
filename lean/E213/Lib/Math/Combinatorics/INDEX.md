...
| `ChainAntichain.lean` | **Mirsky's theorem on `2^[n]` (chain/antichain duality), CLOSED**: `chain_card_inj` (the *chain* SEPARATE — comparable equal-size members coincide, dual of the antichain SEPARATE), `IsChain`, ★ `chain_length_le` (height ≤ `n+1`, size map ↪ `idxList(n+1)`), `initSeg`/`canonChain` + ★ `canonChain_max` (the height is **achieved**: the canonical chain `∅⊂{0}⊂…⊂[n]` is a nodup chain of exactly `n+1`), `chain_layer_cap` (chain meets each layer ≤ once), `mem_own_layer` (the layers cover), ★★ `mirsky_boolean` — longest chain = `n+1` = #layers (the `n+1` size-layers are the minimum antichain partition).  `mem_idxList_of_lt` | ∅-axiom |
