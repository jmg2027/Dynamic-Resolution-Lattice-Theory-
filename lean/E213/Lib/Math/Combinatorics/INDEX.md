...
| `BollobasCount.lean` | **the favour-count injection (the rung's geometry, in progress)**: `weave` (mask-guided interleave) + `weave_perm` (it rearranges `xs++ys`), `before_append_mem`/`weave_preserves_before` (order preservation), ★ `weave_favours` — the woven ordering `weave mask (σA++σB) σR` *favours* `(A,B)` (the geometric heart: the construction lands in the favouring set).  What remains: the enumeration (`weave` injectivity via filter-recovery + the 4-level count = `favourCountTarget`), which closes `bollobas_of_count` unconditionally | ∅-axiom |
