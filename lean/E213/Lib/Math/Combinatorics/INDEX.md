...
| `Permutations.lean` | permutation enumeration with factorial cardinality: `insertEverywhere`, `perms`, ★ `perms_length : (perms l).length = fact l.length` (the `n!` count).  The chain-count infra shared by the named Sperner/Ramsey bounds; reusable for the Leibniz determinant sum | ∅-axiom |
