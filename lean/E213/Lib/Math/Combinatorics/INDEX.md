...
| `Pigeonhole.lean` | no injection `Fin (N+1) → Fin N` (`no_inj_lt`); **constructive collision-finder** `exists_collision`/`exists_collision_lt` (returns the actual colliding pair via a decidable `scan` + `shiftAround`, not just `False`) | ∅-axiom |
| `ErdosSzekeres.lean` | `erdos_szekeres` — a sequence of `>(r−1)(s−1)` distinct values has a strictly-increasing length-`r` or strictly-decreasing length-`s` subsequence, via the `(inc,dec)` label-box pigeonhole (`exists_collision_lt`); explicit subsequence extracted choice-free (`inc_subseq`) | ∅-axiom |
