...
| `JsjDeep.lean` | FW-2 | ~92 | JSJ 3-cell complex Euler-target scaffold + cycle inventory (6 multi-edge 2-cycles + 3 simple 4-cycles = 9 atomic, rank 8) + concrete (k, j) attaching map specifications + bipartite S/T cut → JSJ torus parallel + FW-2 unbounded close + **concrete 3-mfd target attaching maps** (S³: (k,j)=(7,0); T³: (8,1); L(p,q): (10,3)) with `CellComplexK32Attaching` data carrying explicit edge-index 2-cell lists + 2-cell-index 3-cell lists + ★★★★★★★★ `FW2_concrete_attaching_close` |
