...
| `JsjDeep.lean` | FW-2 | ~145 | JSJ 3-cell complex Euler-target scaffold + cycle inventory (6 multi-edge 2-cycles + 3 simple 4-cycles = 9 atomic, rank 8) + concrete (k, j) attaching map specifications + bipartite S/T cut → JSJ torus parallel + FW-2 unbounded close + **concrete 3-mfd target attaching maps** (S³, T³, L(p,q)) + **L(p, q) parameter family** (`Lpq_attaching_pq p q` parametric with modular-q boundaries; L(2,1)=ℝP³, L(3,1), L(5,1), L(5,2), L(7,2), L(7,3); `lensEquiv` distinguishing L(5,1)/L(5,2); `lensTorsionOrder = p`) + **L(p, q) classification refinement** (`lensEquivFull` extending `lensEquiv` with q·q'≡±1 mod p case; L(7,2)≅L(7,4), L(11,3)≅L(11,4) via product-only) + **connected sum** `M₁ # M₂` (`connectedSumShape (k₁+k₂-7, j₁+j₂)` preserves k-j=7; `connectedSumAttaching` cell-level concatenation with drop-7; L(5,1)#L(7,2) at (13,6), T³#T³ at (9,2) both χ=0) + ★★★★★★★★ `FW2_concrete_attaching_close` + ★★★★★★★★ `Lpq_parameter_family_close` + ★★★★★★★★ `connectedSum_and_Lpq_refinement_close` |
| `MetricGeometries.lean` | FW-4 + 8-geo Lie | ~40 | mod-k Möbius P Lens family + F_5 Nil uniqueness + curvature sign / isometry-group dim / Lie group dim per geometry + 6-class Lie partition (semisimple/abelian/nilpotent/solvable/hyperbolic/product) + center-dim totals + ★★★★★★★ `FW4_direct_realization_close` + ★★★★★★★ `eight_geo_lie_group_infra_close` |
