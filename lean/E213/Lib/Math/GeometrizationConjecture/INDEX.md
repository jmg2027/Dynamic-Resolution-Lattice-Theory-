...
| `JsjDeep.lean` | FW-2 | ~124 | JSJ 3-cell complex Euler-target scaffold + cycle inventory (6 multi-edge 2-cycles + 3 simple 4-cycles = 9 atomic, rank 8) + concrete (k, j) attaching map specifications + bipartite S/T cut → JSJ torus parallel + FW-2 unbounded close + **concrete 3-mfd target attaching maps** (S³: (k,j)=(7,0); T³: (8,1); L(p,q): (10,3)) with `CellComplexK32Attaching` data + **L(p, q) parameter family** (`Lpq_attaching_pq p q` parametric attaching with modular-q 3-cell boundaries; instances L(2,1)=ℝP³, L(3,1), L(5,1), L(5,2), L(7,2), L(7,3); `lensEquiv` decidable equivalence relation distinguishing L(5,1)/L(5,2); `lensTorsionOrder = p`) + ★★★★★★★★ `FW2_concrete_attaching_close` + ★★★★★★★★ `Lpq_parameter_family_close` |
