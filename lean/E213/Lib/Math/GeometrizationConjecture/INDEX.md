...
| `JsjDeep.lean` | FW-2 | ~181 | JSJ 3-cell complex Euler-target scaffold + cycle inventory (9 atomic, rank 8) + concrete (k, j) attaching + bipartite S/T → JSJ torus + FW-2 unbounded + concrete 3-mfd target attaching maps (S³, T³, L(p,q)) + L(p, q) parameter family + L(p, q) classification refinement (`lensEquivFull`) + connected sum + PURE universal k − j = 7 preservation + **multi-fold connected sums** (`multiConnectedSumShape` folding `connectedSumShapePair` over a list with `(7,0)` identity; pair/triple/mixed examples) + **Heegaard splitting genus** (`heegaardGenus`: S³→0, T³→3, L(p,q)→1; `heegaardGenusSum` additivity; `multiHeegaardGenus` list version; `isS3_byGenus` decidable Poincaré-style test) + ★★★★★★★★ multiple capstones (FW2_concrete, Lpq_parameter_family, connectedSum_and_Lpq_refinement, connectedSum_universal, multi_fold_connected_sum, heegaard_genus) |
