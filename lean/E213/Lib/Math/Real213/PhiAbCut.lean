...
namespace E213.Lib.Math.Real213.PhiAbCut

open E213.Theory (Raw)
open E213.Lib.Math.Mobius213.Px.FibonacciAtomicLock (fib)
open E213.Lib.Math.Real213.FibCassiniNat (fib_cassini_norm fib_odd_pos cs_eq_phiCut)
...
theorem phiCompletion_limit_eq_phiCut (m k : Nat) :
    phiCompletion.limit m k = E213.Lib.Math.Real213.PhiAsCut.phiCut m k := by
  show phiAb.cut (2*k) m k = E213.Lib.Math.Real213.PhiAsCut.phiCut m k
  rw [phiAb_cut_eq]
  exact cs_eq_phiCut (2*k) m k (Nat.le_refl _)

end E213.Lib.Math.Real213.PhiAbCut
