...
## §3 — π: P-recursive of order 1 with degree-2 coefficients (recurrences exhibited)
  ...
  * `wallis_num_holonomic`, `wallis_den_holonomic` — π's Wallis convergents are
    ...
    wallisDen n·((2n+1)(2n+3))`, with **degree-2** polynomial step coefficients.  So
    π's cross-determinant ratio (their product) is degree 4 — depth 6 (1 cross-det +
    1 ratio + 4 differences), exactly as `DivergenceDepth` records.  By
    `binomCol_polyDepth` a degree-4 polynomial has depth 4; pinning π's quartic onto
    that count is the residual nonlinear-`Nat`-expansion step (no ∅-axiom `ring`).
...
/-- ★★★ **π is P-recursive (holonomic) of order 1.**  Both Wallis convergent
    sequences satisfy first-order recurrences with degree-2 polynomial coefficients —
    the explicit P-recursive data underlying π's depth-6 classification.  (Pinning
    the degree-4 cross-determinant ratio onto `binomCol_polyDepth`'s "degree 4 ⟹
    depth 4" count is a residual nonlinear-`Nat` expansion, there being no ∅-axiom
    `ring`.) -/
theorem pi_is_P_recursive :
  ...
  ⟨wallis_num_holonomic, wallis_den_holonomic⟩
