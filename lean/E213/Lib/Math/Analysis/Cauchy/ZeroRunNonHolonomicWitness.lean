...
namespace E213.Lib.Math.Analysis.Cauchy.ZeroRunNonHolonomicWitness

open E213.Lib.Math.Analysis.Cauchy.ZeroRunNonHolonomic (HomogRec zero_run_not_homogRec)
open E213.Lib.Math.Analysis.Cauchy.NonHolonomicWitness (one_le_pow)
...
open E213.Lib.Math.Analysis.Cauchy.ZeroRunNonHolonomic
  (AutoRec two_continuations_not_autoRec FiniteRecurrence)
...
/-- ★★★ **`χ` escapes every finite-state machine.**  The powers-of-two indicator is generated by
    no finite recurrence of either class — neither time-varying homogeneous P-recursive
    (`chi_nonHolonomic`) nor time-invariant autonomous (`chi_not_autoRec`).  Bounded, computable
    (`chi_values`), yet machine-free: the continued-fraction-scale realisation of the residue
    trace escaping every finite-state machine. -/
theorem chi_not_finiteRecurrence : ¬ FiniteRecurrence chi :=
  fun h => h.elim chi_nonHolonomic chi_not_autoRec

/-! ## §6 — the Morse–Hedlund route, instantiated: `isPow2` is aperiodic -/

open E213.Lib.Math.Analysis.Cauchy.MorseHedlund (EvPeriodic aperiodic_not_autoRec b2n)

theorem isPow2_false_of_chi_zero {n : Nat} (h : chi n = 0) : isPow2 n = false := by
  cases hb : isPow2 n with
  | false => rfl
  | true => exact absurd ((chi_one_of hb).symm.trans h) (by decide)

/-- The powers-of-two indicator is **not eventually periodic**: an all-`false` window of length
    `p` (a gap past `N`) would, by periodicity, force `false` forever — contradicting the powers
    of two past it. -/
theorem isPow2_not_evPeriodic : ¬ EvPeriodic isPow2 := by
  rintro ⟨p, hp, N, hper⟩
  obtain ⟨M, hMN, hMrun0⟩ := chi_runs p N
  have hMrun : ∀ i, i < p → isPow2 (M + i) = false :=
    fun i hi => isPow2_false_of_chi_zero (hMrun0 i hi)
  have hall : ∀ t, isPow2 (M + t) = false := by
    have aux : ∀ T t, t < T → isPow2 (M + t) = false := by
      intro T
      induction T with
      | zero => intro t ht; exact absurd ht (Nat.not_lt_zero t)
      | succ T ihT =>
        intro t ht
        rcases Nat.lt_or_ge t p with htp | hpt
        · exact hMrun t htp
        · have hle : t ≤ T := Nat.le_of_lt_succ ht
          have hge : N ≤ M + (t - p) := Nat.le_trans hMN (Nat.le_add_right M (t - p))
          have hpereq := hper (M + (t - p)) hge
          have hidx : (M + (t - p)) + p = M + t := by
            rw [Nat.add_assoc, E213.Tactic.NatHelper.sub_add_cancel hpt]
          rw [hidx] at hpereq
          rw [hpereq]
          exact ihT (t - p) (Nat.lt_of_lt_of_le (Nat.sub_lt (Nat.lt_of_lt_of_le hp hpt) hp) hle)
    intro t; exact aux (t + 1) t (Nat.lt_succ_self t)
  have hbig := hall (2 ^ M - M)
  rw [E213.Tactic.NatHelper.add_sub_of_le (Nat.le_of_lt (lt_two_pow M))] at hbig
  have htrue : isPow2 (2 ^ M) = true := pow2aux_pow M (2 ^ M) (lt_two_pow M)
  rw [hbig] at htrue
  exact absurd htrue (by decide)

/-- ★★ **A bounded aperiodic witness for the Morse–Hedlund route.**  `isPow2` (the `{0,1}`-valued
    powers-of-two indicator) is generated by no autonomous finite-state machine — via
    aperiodicity, not zero-runs.  (The criterion `aperiodic_not_autoRec` is general: it equally
    rejects *dense* aperiodic sequences such as Thue–Morse, whose own aperiodicity is the only
    missing input.) -/
theorem isPow2_morse_not_autoRec :
    ¬ AutoRec (fun n => ((b2n (isPow2 n) : Nat) : Int)) :=
  aperiodic_not_autoRec isPow2 isPow2_not_evPeriodic

end E213.Lib.Math.Analysis.Cauchy.ZeroRunNonHolonomicWitness
