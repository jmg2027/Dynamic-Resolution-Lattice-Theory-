...
/-! ### §3 — Self-referential twisted Leibniz at `(2, 1)` on Δ³

Same self-referential structure as `(1, 1)` extended to bidegree
`(k, l) = (2, 1)`.  Confirms the general conjecture across two
structurally-distinct bidegrees.

We use Δ³ (`n = 4`) rather than Δ⁴ to keep `decide`'s search
space tractable:
  ...
  · middle-removed face: `τ \ {τ[2]} = [0,1,3]` in
    `Fin (binom 4 3)`
...
/-- The middle-removed face map at `(k, l, n) = (2, 1, 4)`.
    Trivially single-valued since `binom 4 4 = 1`. -/
...
/-- **Self-referential twisted Leibniz at `(2, 1)` on Δ³** —
    ...
    Decide-verified across 2¹⁰ · 1 = 1024 cases on Δ³.

    With both `(1, 1)` and `(2, 1)` decide-verified, the
    self-referential twisted Leibniz
    `δ(α⌣β) = δα⌣β ⊕ α⌣δβ ⊕ (α⌣β)|_{face_k}` is empirically
    validated at two structurally-distinct bidegrees.  The pattern
    is independent of the specific bidegree — only the middle
    position `k` varies.  `(2, 2)` requires 2²⁰ · 1 ≈ 1M decide
    cases at `maxHeartbeats` and is the natural next verification
    target. -/
