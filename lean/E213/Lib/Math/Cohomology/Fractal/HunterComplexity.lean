...
## Hierarchy
  ...
  · **depth ≥ 2**: catalogue atoms `{13, 521}` (neither appears in
    depth-1).
  · **depth ≥ 3**: catalogue atom `{521}` (does not appear in
    depth-2 restricted; full depth-2 incl. outer pow is excluded
    via the algebraic argument that `521` is prime and not in
    depth-1).
...
/-! ## §4 Atomic-prime complexity classification
    ...
    For each catalogue prime `p ∈ {2, 3, 5, 7, 13, 521}`, we record
    its `inDepth1` status and (for primes outside depth-1) its
    depth-≥-2 positive witness. -/
...
/-! ## §8 Restricted depth-2 lower bound for `521`
