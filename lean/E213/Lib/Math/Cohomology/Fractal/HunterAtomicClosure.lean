...
contains a **structural FLT sub-closure** of 15 pairs via Fermat's
Little Theorem: for every `p ∈ cat` and `a ∈ cat` with `a < p`,
`a^p % p = a`.  Since all six catalogue elements are prime, FLT
...
operation are reported (out of 36 = 6² ordered pairs).
...
def inCat (n : Nat) : Bool :=
  n == 2 || n == 3 || n == 5 || n == 7 || n == 13 || n == 521

theorem cat_length : cat.length = 6 := rfl
...
/-! ## §4 Explicit closure-count theorems
    ...
    Out of 36 = 6² ordered pairs per modulus, count the catalogue
    hits.  All values `decide`-verified. -/

/-- Addition closure counts per `p ∈ cat`. -/
theorem closure_add_2   : closureCountAdd 2   = 0  := by decide
theorem closure_add_3   : closureCountAdd 3   = 10 := by decide
theorem closure_add_5   : closureCountAdd 5   = 13 := by decide
theorem closure_add_7   : closureCountAdd 7   = 18 := by decide
theorem closure_add_13  : closureCountAdd 13  = 15 := by decide
theorem closure_add_521 : closureCountAdd 521 = 14 := by decide

/-- Multiplication closure counts per `p ∈ cat`. -/
theorem closure_mul_2   : closureCountMul 2   = 0  := by decide
theorem closure_mul_3   : closureCountMul 3   = 12 := by decide
theorem closure_mul_5   : closureCountMul 5   = 8  := by decide
theorem closure_mul_7   : closureCountMul 7   = 10 := by decide
theorem closure_mul_13  : closureCountMul 13  = 10 := by decide
theorem closure_mul_521 : closureCountMul 521 = 0  := by decide

/-- Exponentiation closure counts per `p ∈ cat`. -/
theorem closure_pow_2   : closureCountPow 2   = 0  := by decide
theorem closure_pow_3   : closureCountPow 3   = 15 := by decide
theorem closure_pow_5   : closureCountPow 5   = 20 := by decide
theorem closure_pow_7   : closureCountPow 7   = 16 := by decide
theorem closure_pow_13  : closureCountPow 13  = 8  := by decide
theorem closure_pow_521 : closureCountPow 521 = 5  := by decide
...
/-- Addition non-closure: `(3 + 7) % 13 = 10`, not in `cat`. -/
theorem add_non_closure : inCat ((3 + 7) % 13) = false := by decide
...
/-! ## §6 FLT sub-closure — the structural finding
    ...
    a FLT-closure pair.  Total: `0 + 1 + 2 + 3 + 4 + 5 = 15` FLT pairs.
...
/-- FLT bundle at `p = 521`: `a ∈ {2, 3, 5, 7, 13}`. -/
theorem flt_at_521 :
    powMod 2 521 521 = 2 ∧ powMod 3 521 521 = 3 ∧ powMod 5 521 521 = 5
    ∧ powMod 7 521 521 = 7 ∧ powMod 13 521 521 = 13 := by decide

/-- Count of FLT-closure pairs: `0 + 1 + 2 + 3 + 4 + 5 = 15`. -/
def fltClosureCount : Nat := 15

theorem fltClosureCount_value : fltClosureCount = 15 := rfl

/-! ## §7 Capstone

    Bundled verdict: catalogue is **not** closed under any of
    `+`, `*`, `^` mod `p`, but contains a **structural FLT
    sub-closure** of 15 pairs. -/
