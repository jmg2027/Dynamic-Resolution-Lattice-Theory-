...
/-!
# Unrestricted depth-2 cut-off — outer pow case for catalogue prime 521

Extends `AurifeuilleanDepth2Cutoff.lean` (which handled outer ∈
{+, *}) to the **outer = pow** case for the catalogue prime
`521`, completing the depth-2 cut-off for this atom.
...
We show no such `a^b` equals `521` by splitting on `b`:
  ...
  · **Large range** `b ∈ {15, 25, 27, 32, 125, 243, 3125}`: use
    monotonicity `a^b ≥ 2^15 = 32768 > 521`.
...
value either:
  ...
  · is at most `9 765 625` (outer ∈ {+, *}), so we need explicit
    enumeration to exclude `521`; this is
    `HunterComplexity.complexity_521_at_least_3_restricted`;
    OR

  · equals `a^b` for `a, b ∈ depth1Universe` (outer = ^), and this
    file shows that fails for `521`.

Hence the unrestricted-depth-2 cut-off for `521` is closed.
-/
...
/-! ## §3 Monotonicity for the large range
    ...
    For `a ≥ 2` and `b ≥ 15`, `a^b ≥ 2^15 = 32768 > 521`. -/
...
/-- Corollary: `a^b ≠ 521` for `a ≥ 2`, `b ≥ 15`. -/
theorem large_b_ne_521 (a b : Nat) (ha : 2 ≤ a) (hb : 15 ≤ b) :
    a ^ b ≠ 521 := by
...
/-! ## §5 Main theorem — `521 ∉ depth-2 outer-pow` -/
...
theorem depth2_pow_ne_521 :
  ...
  · have ha : 2 ≤ D1 i := D1_ge_2 i
    ...
    exact large_b_ne_521 (D1 i) (D1 j) ha hb
...
/-- ★★★★★ **Capstone — unrestricted depth-2 cut-off for catalogue
    ...
    Combines:
      (1) restricted depth-2 (outer ∈ {+, *}, from
          `HunterComplexity.complexity_521_at_least_3_restricted`),
      (2) outer-pow case (this file's `depth2_pow_ne_521`).

    Hence `hunterComplexity(521) = 3` (full, no restriction),
    promoting the previously restricted lower bound to the
    unrestricted bound.

    Upper bound: explicit depth-3 witness
    `521 = 2^(3^2) + 3^2` from `HunterComplexity`. -/
theorem capstone :
