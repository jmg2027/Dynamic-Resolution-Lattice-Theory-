...
namespace E213.Lib.Math.Cohomology.HodgeConjecture.Pairing.SignatureMetaTheorem
...
def T2n_inductive :=
  E213.Lib.Math.Cohomology.HodgeConjecture.Pairing.T2nInductive.T2n_inductive_pattern_theorem

/-- Σ_g surface parametric signature theorem.  Signature on H¹(Σ_g; ℤ)
    is `(g, g)` for all `g ≥ 0`, with connected-sum additivity. -/
def Sigma_g_master :=
  E213.Lib.Math.Cohomology.HodgeConjecture.Pairing.GenusGSurface.genus_g_signature_master

/-- Künneth tensor signature theorem.  Per-Künneth-piece signature
    multiplicativity `(p·p' + q·q', p·q' + q·p')`. -/
def Tensor_master :=
  E213.Lib.Math.Cohomology.HodgeConjecture.Pairing.TensorSignature.tensor_signature_kunneth_master
...
end E213.Lib.Math.Cohomology.HodgeConjecture.Pairing.SignatureMetaTheorem
