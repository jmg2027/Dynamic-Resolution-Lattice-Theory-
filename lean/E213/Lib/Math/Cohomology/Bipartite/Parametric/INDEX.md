...
| `BettiOneUniversal.lean` | — | 2 | **Universal first Betti number `b₁ = E − V + 1`** from ∅-axiom `BoolEnum` cardinalities (`\|C⁰\| = 2^V`, `\|ker δ⁰\| = 2`, `\|im δ⁰\| = 2^(V−1)`) + rank-nullity/first-iso arithmetic.  `betti_one_universal` (parametric) + `betti_one_K32` (`b₁ = 8 = NS² − 1 = 1/α₃`) |
