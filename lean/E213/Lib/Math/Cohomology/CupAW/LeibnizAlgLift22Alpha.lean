...
/-!
# Algebraic Leibniz lift at (5, 2, 2) — α-side lens (b=2 corollary)

Thin corollary of `LeibnizAlgLiftAlpha.leibniz_via_α_decomp_general`
at `b = 2`.  At that value the `castA`/`castB` casts in the general
helper's signature reduce to identity (both index types become
`Fin (binom 5 4)`), so the user's `h_components` (no cast) matches
directly.
-/
...
/-- ★ α-decomp lens at (5, 2, 2).  PURE.  Corollary of
    `LeibnizAlgLiftAlpha.leibniz_via_α_decomp_general` at `b=2`. -/
