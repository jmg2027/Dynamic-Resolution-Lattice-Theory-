...
namespace E213.Lib.Math.Cohomology.CupAW.LeibnizMid
...
/-- ★★★ Universal Leibniz Prop-lift at (4, 1, 1) — AW cup.
    ∅-axiom — chains `pattern_eq_at` (pointwise) through
...
theorem leibniz_universal_4_1_1
  ...
  let hδβ : ∀ j, delta β j = delta pβ j :=
    fun j => E213.Lib.Math.Cohomology.Delta.Pointwise.delta_pointwise_eq
              β pβ hβ j
  let h_cup : ∀ j, cupAW 4 1 1 α β j = cupAW 4 1 1 pα pβ j :=
    fun j => E213.Lib.Math.Cohomology.CupAW.Pointwise.cupAW_pointwise_eq
              α pα β pβ hα hβ j
  let h_lhs : delta (cupAW 4 1 1 α β) i = delta (cupAW 4 1 1 pα pβ) i :=
    E213.Lib.Math.Cohomology.Delta.Pointwise.delta_pointwise_eq
      (cupAW 4 1 1 α β) (cupAW 4 1 1 pα pβ) h_cup i
  let h_rhs1 : cupAW 4 2 1 (delta α) β i = cupAW 4 2 1 (delta pα) pβ i :=
    E213.Lib.Math.Cohomology.CupAW.Pointwise.cupAW_pointwise_eq
      (delta α) (delta pα) β pβ hδα hβ i
  let h_rhs2 : cupAW 4 1 2 α (delta β) i = cupAW 4 1 2 pα (delta pβ) i :=
    E213.Lib.Math.Cohomology.CupAW.Pointwise.cupAW_pointwise_eq
      α pα (delta β) (delta pβ) hα hδβ i
  let h_pat := leibniz_pattern_4_1_1
                  (α ⟨0, by decide⟩) (α ⟨1, by decide⟩)
                  (α ⟨2, by decide⟩) (α ⟨3, by decide⟩)
                  (β ⟨0, by decide⟩) (β ⟨1, by decide⟩)
                  (β ⟨2, by decide⟩) (β ⟨3, by decide⟩) i
  let h_xor_eq : xor (cupAW 4 2 1 (delta pα) pβ i) (cupAW 4 1 2 pα (delta pβ) i)
               = xor (cupAW 4 2 1 (delta α) β i) (cupAW 4 1 2 α (delta β) i) :=
    by rw [h_rhs1, h_rhs2]
  h_lhs.trans (h_pat.trans h_xor_eq)

end E213.Lib.Math.Cohomology.CupAW.LeibnizMid
