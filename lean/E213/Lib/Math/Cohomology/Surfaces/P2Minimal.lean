...
open E213.Meta.Int213 (int_sq_nonneg mul_eq_zero)
...
/-- The cup-pairing is positive-**semidefinite**: `Q(α,α) = (α·line)² ≥ 0` for every class
    (`H²(ℙ²)=ℤ` is 1-dimensional, generated by `H`, so `α = (α line)·H`). -/
theorem cup_self_nonneg (α : C2) : 0 ≤ cup α α Cell4.plane :=
  int_sq_nonneg (α Cell2.line)

/-- ★★★★★ **Signature `(1, 0)` of `H²(ℙ²; ℤ)`** — ∅-axiom.  The intersection form is
    positive-definite of rank 1: the hyperplane class `H` is positive (`H⌣H = +1`), the
    form is positive-semidefinite (`cup_self_nonneg`), and it is null only at `0`
    (`(α line)² = 0 ⟹ α line = 0`).  Since `H²(ℙ²) = ℤ` is 1-dimensional (single 2-cell
    `line`), this is signature `(1, 0)` — the `decide`-witness analogue of
    `T2Minimal.Signature.signature_one_one_witness`. -/
theorem signature_one_zero_witness :
    cup hyperplane_class hyperplane_class Cell4.plane = 1
    ∧ (∀ α : C2, 0 ≤ cup α α Cell4.plane)
    ∧ (∀ α : C2, cup α α Cell4.plane = 0 → α Cell2.line = 0) :=
  ⟨cup_HH, cup_self_nonneg, fun α h => by
    have : α Cell2.line * α Cell2.line = 0 := h
    rcases mul_eq_zero this with h1 | h1 <;> exact h1⟩
