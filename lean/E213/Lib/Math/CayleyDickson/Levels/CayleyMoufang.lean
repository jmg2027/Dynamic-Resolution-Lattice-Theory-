...
private theorem cay_self_mul_conj (u : Cayley) :
    u * Cayley.conj u = ofInt (E213.Lib.Math.CayleyDickson.Levels.CayleyHeavy.normSq u) := by
...
instance : MoufangIntegerNormed213 Cayley where
  ofInt               := ofInt
  normSq              := E213.Lib.Math.CayleyDickson.Levels.CayleyHeavy.normSq
...
theorem moufang_normSq_mul (u v : Cayley) :
    E213.Lib.Math.CayleyDickson.Levels.CayleyHeavy.normSq (u * v) = E213.Lib.Math.CayleyDickson.Levels.CayleyHeavy.normSq u * E213.Lib.Math.CayleyDickson.Levels.CayleyHeavy.normSq v :=
