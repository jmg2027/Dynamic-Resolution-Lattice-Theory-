...
namespace E213.Math.DyadicFSM.ForwardClosure

open E213.Math.DyadicFSM.Signature (signature nextVertex)
...
private theorem sub_lt_of_lt_add {a b c : Nat} (hca : c ≤ a)
    (h : a < b + c) : a - c < b := by
  have h1 : a - c + c = a := E213.Tactic.Nat213.sub_add_cancel hca
...
theorem sub_is_multiple_of_p (i j p : Nat) (hp : 0 < p)
  ...
  have h1 : (j - i) % p = 0 := by
    ...
    have hadd : j % p = (i % p + (j - i)) % p := by
      have h := E213.Math.NatHelpers.AddMod213.add_mod_left hp i (j - i)
      rw [hji] at h
      exact h
    rw [hmod] at hadd
    have hadd2 : (j % p + (j - i)) % p
                = (j % p % p + (j - i) % p) % p :=
      E213.Math.NatHelpers.AddMod213.add_mod hp _ _
    rw [E213.Math.NatHelpers.AddMod213.mod_mod] at hadd2
    rw [hadd2] at hadd
    ...
    · -- (j-i)%p = 0 directly
      ...
      exact (E213.Tactic.Nat213.add_left_cancel
              (h0.trans hadd)).symm
    · exfalso
      ...
      have h_add_p : j % p + p = j % p + (j - i) % p :=
        (congrArg (· + p) hadd).trans
          (E213.Tactic.Nat213.sub_add_cancel hsum_ge)
      have h_p_eq : p = (j - i) % p :=
        E213.Tactic.Nat213.add_left_cancel h_add_p
  ...
  have h2 : p * ((j - i) / p) + (j - i) % p = j - i :=
    E213.Math.NatHelpers.AddMod213.div_add_mod (j - i) p
...
end E213.Math.DyadicFSM.ForwardClosure
