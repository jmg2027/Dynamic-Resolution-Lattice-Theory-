...
def CauchyCutSeq.cutSum (a b : CauchyCutSeq) : CauchyCutSeq where
  cs := fun i => E213.Math.Real213.CutSum.cutSum (a.cs i) (b.cs i)
  N := fun m k => Nat.max (maxModulus a.N k (2*m))
                          (maxModulus b.N k (2*m))
  cauchy := by
    intro m k i j hi hj
    show E213.Math.Real213.CutSum.cutSum (a.cs i) (b.cs i) m k
       = E213.Math.Real213.CutSum.cutSum (a.cs j) (b.cs j) m k
...
theorem CauchyCutSeq.cutSum_limit (a b : CauchyCutSeq) :
    (a.cutSum b).limit = E213.Math.Real213.CutSum.cutSum a.limit b.limit := by
  funext m k
...
end E213.Math.Real213.CauchyArithSum
