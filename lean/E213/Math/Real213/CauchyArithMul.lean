...
def CauchyCutSeq.cutMul (a b : CauchyCutSeq) : CauchyCutSeq where
  cs := fun i => E213.Math.Real213.CutSum.cutMul (a.cs i) (b.cs i)
  N := fun m k => Nat.max (maxNAt a.N k ((m+1)*(k+1)))
                          (maxNAt b.N k ((m+1)*(k+1)))
  cauchy := by
    intro m k i j hi hj
    show E213.Math.Real213.CutSum.cutMul (a.cs i) (b.cs i) m k
       = E213.Math.Real213.CutSum.cutMul (a.cs j) (b.cs j) m k
...
theorem CauchyCutSeq.cutMul_limit (a b : CauchyCutSeq) :
    (a.cutMul b).limit = E213.Math.Real213.CutSum.cutMul a.limit b.limit := by
  funext m k
...
end E213.Math.Real213.CauchyArithMul
