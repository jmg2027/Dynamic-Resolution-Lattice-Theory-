...
* `b_1(K_{3,2}^{(2)}) = 8` is now derived two ways:
  - **Scalar**: `Physics.PhotonKernel.b_1_eq_8` (Euler formula
    E − V + 1 = 12 − 5 + 1 = 8).
  - **Chain-level**: `E213.Math.Cohomology.Bipartite.V32Betti.b1_eq_8_dim_count` (rank-nullity from
...
namespace E213.Math.Cohomology.AlphaEMBridge
...
theorem b1_two_derivations_agree :
    b_1 = E213.Math.Cohomology.Bipartite.V32Betti.kerSizeDelta0 * 4
    ∧ b_1 = NS * NS - 1
    ∧ E213.Math.Cohomology.Bipartite.V32Betti.kerSizeDelta0 = 2 := by
  refine ⟨?_, ?_, ?_⟩
  · decide
  · exact b_1_eq_8.trans (by decide : (8 : Nat) = NS * NS - 1)
  · exact E213.Math.Cohomology.Bipartite.V32Betti.kerSizeDelta0_eq_2
...
end E213.Math.Cohomology.AlphaEMBridge
