...
theorem pellProper3_run_period_8 :
    ∀ k, (pellProperFSMmod 3 (by omega)).run (k + 8)
        = (pellProperFSMmod 3 (by omega)).run k := by
  intro k
  induction k with
  | zero => decide
  | succ k' ih =>
    show (pellProperFSMmod 3 (by omega)).step _
        = (pellProperFSMmod 3 (by omega)).step _
    rw [ih]

/-- ★★★ Pell proper mod 5: period 12 = 2(p+1) (inert). -/
theorem pellProper5_run_period_12 :
    ∀ k, (pellProperFSMmod 5 (by omega)).run (k + 12)
        = (pellProperFSMmod 5 (by omega)).run k := by
  intro k
  induction k with
  | zero => decide
  | succ k' ih =>
    show (pellProperFSMmod 5 (by omega)).step _
        = (pellProperFSMmod 5 (by omega)).step _
    rw [ih]

/-- ★★★ Pell proper mod 7: period 6 = p-1 (split). -/
theorem pellProper7_run_period_6 :
    ∀ k, (pellProperFSMmod 7 (by omega)).run (k + 6)
        = (pellProperFSMmod 7 (by omega)).run k := by
  intro k
  induction k with
  | zero => decide
  | succ k' ih =>
    show (pellProperFSMmod 7 (by omega)).step _
        = (pellProperFSMmod 7 (by omega)).step _
    rw [ih]
