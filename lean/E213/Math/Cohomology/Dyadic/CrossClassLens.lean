...
namespace E213.Math.Cohomology.Dyadic.CrossClassLensopen E213.Math.Cohomology.Dyadic.ArithFSM (pellFSMmod3)
...
theorem tribMod2_BitFSM_bits_period_4 :
    ∀ k, (tribFSMmod2.toBitFSM (by decide : (0:Nat) < 2)).bits (k + 4)
        = (tribFSMmod2.toBitFSM (by decide : (0:Nat) < 2)).bits k := by
  intro k
...
theorem crossLens_pell3_trib2_period_4 :
    ∀ k, (BitFSM.product (n := 9) (m := 8) (by decide)
            ...
            (tribFSMmod2.toBitFSM (by decide))
            xor).bits (k + 4)
