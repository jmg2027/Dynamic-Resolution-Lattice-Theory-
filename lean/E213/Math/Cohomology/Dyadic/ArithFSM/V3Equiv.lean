...
theorem encode3_mod_n {n : Nat} (a b c : Fin n) :
  ...
  have hreshape : a.val * (n * n) + b.val * n + c.val
    ...
    rw [show a.val * (n * n) = a.val * n * n from
        ...
        ← add_mul_213]
  rw [hreshape]
  exact E213.Math.EncodePair213.encode_mod hn (a.val * n + b.val) c.val c.isLt
...
theorem encode3_mod_nn_pub {n : Nat} (a b c : Fin n) :
  ...
  rw [Nat.add_assoc]
  exact E213.Math.EncodePair213.encode_mod hnn a.val (b.val * n + c.val) hbcs
...
theorem encode3_div_nn_pub {n : Nat} (hn : 0 < n) (a b c : Fin n) :
  ...
  have hbcs : b.val * n + c.val < n * n := bn_plus_c_lt_nn b c
  rw [Nat.add_assoc]
  exact E213.Math.EncodePair213.encode_div hnn a.val (b.val * n + c.val) hbcs
...
theorem encode3_inner_div_pub {n : Nat} (hn : 0 < n) (a b c : Fin n) :
  ...
  exact E213.Math.EncodePair213.encode_div hn b.val c.val c.isLt
