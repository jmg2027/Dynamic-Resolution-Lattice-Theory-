import E213.Math.Cohomology.DyadicSignatureBipartite
...
theorem signature_S_count : E213.Physics.Simplex.NS = 3 := rfl

/-- ★ K_{3,2}^{(2)} signature uses exactly NT T-vertices. -/
theorem signature_T_count : E213.Physics.Simplex.NT = 2 := rfl

/-- ★ Total signature dimension = d = 5. -/
theorem signature_d_count : E213.Physics.Simplex.d = 5 := rfl

/-- ★★ Signature S-side index range matches NS. -/
theorem isS_iff_lt_NS (v : Fin 5) :
    isS v ↔ v.val < E213.Physics.Simplex.NS := by
  unfold isS
  show v.val < 3 ↔ v.val < E213.Physics.Simplex.NS
  rw [show E213.Physics.Simplex.NS = 3 from rfl]

/-- ★★ Signature T-side index range matches NT (offset). -/
theorem isT_iff_ge_NS (v : Fin 5) :
    isT v ↔ v.val ≥ E213.Physics.Simplex.NS := by
  unfold isT
  show v.val ≥ 3 ↔ v.val ≥ E213.Physics.Simplex.NS
  rw [show E213.Physics.Simplex.NS = 3 from rfl]
...
theorem signature_NS_NT_alternation (bs : Nat → Bool) (k : Nat) :
    (k % 2 = 0 → (signature bs k).val < E213.Physics.Simplex.NS)
    ∧ (k % 2 = 1 → (signature bs k).val ≥ E213.Physics.Simplex.NS) := by
...
theorem signature_atomicity_capstone :
    E213.Physics.Simplex.NS = 3 ∧ E213.Physics.Simplex.NT = 2
    ∧ E213.Physics.Simplex.d = 5 ∧ E213.Physics.Simplex.NS + E213.Physics.Simplex.NT
