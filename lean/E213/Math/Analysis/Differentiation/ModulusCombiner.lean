...
def combineModulus {xs ys : Nat → Raw}
  ...
  cauchy_at := by
    ...
    have hi_x : i ≥ mod_x.N (mc.precX m k).1 (mc.precX m k).2 :=
      Nat.le_trans (E213.Math.NatHelpers.Max213.le_max_left _ _) hi
    have hi_y : i ≥ mod_y.N (mc.precY m k).1 (mc.precY m k).2 :=
      Nat.le_trans (E213.Math.NatHelpers.Max213.le_max_right _ _) hi
    have hj_x : j ≥ mod_x.N (mc.precX m k).1 (mc.precX m k).2 :=
      Nat.le_trans (E213.Math.NatHelpers.Max213.le_max_left _ _) hj
    have hj_y : j ≥ mod_y.N (mc.precY m k).1 (mc.precY m k).2 :=
      Nat.le_trans (E213.Math.NatHelpers.Max213.le_max_right _ _) hj
...
end E213.Math.Analysis.Differentiation.ModulusCombiner
