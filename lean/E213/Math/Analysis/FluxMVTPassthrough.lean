...
Sub-namespaces preserved (cross-file `open` declarations stay valid):
  ...
  * `E213.Math.Analysis.FluxMVTPassthrough`   — the general theorem
...
(Consolidated 2026-05-05 from 3 phase files: FluxMVTPassthrough
[Phase BF] + FluxMVTClosure [Phase BI] + FluxMVTApplications [Phase BG].
Per-stage capstones dropped.)
-/

namespace E213.Math.Analysis.FluxMVTPassthrough
...
open E213.Math.Analysis.FluxCochain.FluxCut (fluxAlong fluxAlong_id)
...
theorem ftc_bridge_passthrough_unit_pure
  ...
  E213.Math.Analysis.FluxMVT.FluxCut.fluxBalance_trans
    (mvt_passthrough_unit_pure f h_left h_right)
    (E213.Math.Analysis.FluxMVT.FluxCut.fluxBalance_symm _ _
      (fluxAlong_passthrough_unit_pure f h_left h_right))

end FluxCut

end E213.Math.Analysis.FluxMVTPassthrough
...
open E213.Math.Analysis.FluxMVTPassthrough.FluxCut
...
open E213.Math.Analysis.FluxMVT.FluxCut (fluxCutEq)
open E213.Math.Analysis.FluxMVTPassthrough.FluxCut (mvt_passthrough_unit_pure)
open E213.Math.Real213.CutMulOne (cutMul_one_one_at cutMul_one_const_at)
open E213.Math.Real213.CutSumZero (cutMul_zero_zero_at)
...
theorem mvt_cube_via_passthrough_pure :
  ...
  mvt_passthrough_unit_pure (fun x => cutMul x (cutMul x x))
    (E213.Math.Analysis.FluxMVTPolynomial.FluxCut.fluxAlong_cube_unitBracket_backward_at)
    (E213.Math.Analysis.FluxMVTPolynomial.FluxCut.fluxAlong_cube_unitBracket_forward_at)
