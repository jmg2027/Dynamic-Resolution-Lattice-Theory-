...
namespace E213.Math.Analysis.IntegralProperties
...
open E213.Math.Analysis.Antiderivative
  (IsAntiderivative)
    ...
    integral (add_anti id_anti id_anti) db
      = ({ forward := cutSum (db.rightCut) (db.rightCut),
           backward := cutSum (db.leftCut) (db.leftCut) }
              : E213.Math.Analysis.FluxCut.FluxCut)
...
end E213.Math.Analysis.IntegralProperties
