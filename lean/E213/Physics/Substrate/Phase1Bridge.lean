...
namespace E213.Physics.Substrate.Phase1Bridge

open E213.Physics.Substrate.Edges
open E213.Physics.Simplex.Counts

/-- NS_atomic (Phase 2) = NS (Phase 1).  Both are 3. -/
theorem NS_match : NS_atomic = E213.Physics.Simplex.Counts.NS := by decide
...
theorem edges_match :
    num_directed_edges = E213.Physics.Couplings.PhotonKernel.num_edges := by
  decide

/-- Phase 2 cycle space dim = Phase 1 b_1. -/
theorem cycle_space_match :
    num_directed_edges - 5 + 1 = E213.Physics.Couplings.PhotonKernel.b_1 := by
  decide

/-- Phase 2 NS² - 1 = Phase 1 photon kernel. -/
theorem photon_kernel_match :
    NS_atomic * NS_atomic - 1 = E213.Physics.Couplings.PhotonKernel.b_1 := by
  decide
...
theorem bridge_capstone :
    (NS_atomic = E213.Physics.Simplex.Counts.NS)
    ∧ (c_lattice = 2)
    ∧ (num_directed_edges = E213.Physics.Couplings.PhotonKernel.num_edges)
    ∧ (num_directed_edges = 12)
    ∧ (num_directed_edges - 5 + 1 = E213.Physics.Couplings.PhotonKernel.b_1)
    ∧ (NS_atomic * NS_atomic - 1 = 8) := by
  refine ⟨?_, ?_, ?_, ?_, ?_, ?_⟩
  all_goals decide

end E213.Physics.Substrate.Phase1Bridge
