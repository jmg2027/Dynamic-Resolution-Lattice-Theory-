...
A command-level macro that registers a `R4Codomain (ZSqrt D)`
...
```
open E213.Tactic
quad_extension 11       -- registers R4Codomain (ZSqrt 11)
quad_extension 13       -- registers R4Codomain (ZSqrt 13)
quad_extension 17       -- registers R4Codomain (ZSqrt 17)
```

## Diagnostic

`D = 0` is rejected: `ℤ[√0] = ℤ` has no nontrivial involution
and therefore fails R4 (conj would equal id).  Negative D is
inexpressible at the `num` syntax level.

## Scope

Purely a convenience macro on top of the parametric
`ZSqrt.R4_of_pos` theorem.  Adds **no** new mathematical
content; the elaboration resolves to the same `R4_of_pos`
call at every site.
-/
