...
class HasDistinguishing (α : Type) where
  ...
  distinct : a ≠ b
  combine : α → α → α
  /-- Reading-sameness on the codomain (default: Lean `=`).  For codomains whose
      `=` is funext-bearing (`Lens β`, function spaces) an instance overrides this
      with the pointwise sameness (`Lens.eqPW` / `Lens.sameLens`), so `combine_sym`
...
/-- Universal morphism preserves slash, up to the codomain's reading-sameness
    ...
    the literal `=` homomorphism (definitionally, at default transparency); at a
    `same`-overriding codomain (`Lens β` with `eqPW`) it is the ∅-axiom pointwise
    form. -/
...
`Lens/Morphism/{NoDepthParity, DepthParityNotFold, SlashCharNotFold}.lean`.
