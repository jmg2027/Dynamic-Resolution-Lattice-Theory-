...
| `^`-wall diagnosed via "logarithm" | "the log isn't a discrete lattice"; "log₂3 escapes" — naming the `^`-inverse answer *logarithm* and reasoning from its analytic properties | The logarithm is an **exterior ruler** (imported analysis).  The `^`-inverse answer is internally the **cut of the slot question `aˣ=b`** (`Real213`, a residue-internal pointing); whether it folds to a finite tuple is decided in ℕ alone: `aˣ=b` folds ⟺ `exp(a) ∥ exp(b)` (prime-exponent collinearity, `vp_pow`/`vp_mul`), non-fold proved by `TwoThreeUnique.two_three_unique` (exp 2 ∦ exp 3), never by a property of an imported log.  Sits with "External-ruler smuggling" / "Transcendental-as-exterior".  Canonical: frontier `numbersystem_square.md` "The fold-back criterion as exponent-lattice collinearity". |
