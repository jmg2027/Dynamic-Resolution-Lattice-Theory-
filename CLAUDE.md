...
| Breadth-evidence read as confirmation (convergence vs vacuity) | collecting many fields/voices that "also ground on difference" and reading the chorus as support; "more opinions → stronger" | The **cheap first half** ("difference is primitive") is presupposed by *any* conceptual activity → true of every field → **zero discriminating power** (a deepity-amplifier; more confirmations of an unfalsifiable claim raise no posterior).  Evidence lives **only in the second half**: does the field *independently* hit a **completeness-forced un-capturable remainder, with a construction** (the diagonal/`object1_not_surjective` signature)?  So **stratify on the discriminator, don't multiply the universal**: sample fields built to claim **total closure** (complete classifications, decidable/complete theories, total non-contextual models) and **hunt disconfirmation** — "exhibit a faithful completed self-cover that leaves *no* remainder."  The finding is the *asymmetry* (totality-built fields — finite simple groups, Presburger, KS — fail to be total), not the chorus.  Strong convergences are *structural*: homology's `coker`/graded-obstruction tower, Kolmogorov/Chaitin = the *same* Lawvere diagonal, Saussurean *valeur* = `Object1 r=(·=r)`.  Still **suffices-by-breadth (§7.1), not uniqueness**.  Canonical: `frontiers/the_one_act.md` Round 4. |
