...
figures, (vi)~$3+1$ spacetime structure from the hinge-opposite
duality ($1:6:3$ curvature modes uniquely select $(3,2)$),
and (vii)~the impossibility of maintaining spacetime
...
%% ==============================================================
\section{Hinge-opposite duality and spacetime structure}
\label{sec:hinge-opposite}
%% ==============================================================

\subsection{The opposite element theorem}

In a single 4-simplex $\Delta^4$ with 5 vertices, each
triangular hinge $H = \{i,j,k\}$ has an \emph{opposite edge}
$H^\perp = \{l,m\}$ formed by the two vertices not in $H$.
The deficit angle at $H$ measures curvature in the plane
perpendicular to $H$; this direction is determined by the
character of $H^\perp$.

\begin{theorem}[Hinge-opposite curvature classification]
\label{thm:opposite}
In a 4-simplex with local $(3,2)$ partition, the 10 hinges
decompose by opposite-edge type as follows:
\begin{center}
\begin{tabular}{lccll}
\toprule
Hinge type & Opposite type & Count & Curvature direction & Formula \\
\midrule
$AAA$ & $BB$ (temporal) & 1 & temporal ($R_{00}$) & $\binom{\nT}{2}$ \\
$AAB$ & $AB$ (mixed)    & 6 & mixed ($R_{0i}$)    & $\nS\cdot\nT$ \\
$ABB$ & $AA$ (spatial)  & 3 & spatial ($R_{ij}$)   & $\binom{\nS}{2}$ \\
\midrule
Total &                  & 10 &                     & $\binom{5}{3}$ \\
\bottomrule
\end{tabular}
\end{center}
The perpendicular direction to a hinge is spanned by the
opposite edge; its $S/T$ character determines whether the
deficit angle encodes temporal, mixed, or spatial curvature.
\end{theorem}

\begin{proof}
A hinge with $s$ $A$-vertices and $(3-s)$ $B$-vertices has
opposite edge containing $(n_S - s)$ $A$-vertices and
$(n_T - 3 + s)$ $B$-vertices. With $(n_S,n_T)=(3,2)$:
$BB$ opposite requires $s = n_S = 3$, giving $\binom{3}{3}\binom{2}{0} = 1$ hinge;
$AB$ opposite requires $s = n_S-1 = 2$, giving $\binom{3}{2}\binom{2}{1} = 6$;
$AA$ opposite requires $s = n_S-2 = 1$, giving $\binom{3}{1}\binom{2}{2} = 3$.
A $BBB$ hinge would require $s = 0$ with $\binom{2}{3} = 0$:
it is combinatorially forbidden.
\end{proof}

\begin{corollary}[$3+1$ spacetime from $(3,2)$]
\label{cor:31}
The curvature mode count $1:6:3$ (temporal : mixed : spatial)
is the unique signature of $(3,2)$.  No other partition of 5
vertices produces exactly 1 temporal and 3 spatial curvature
modes:
\begin{center}
\begin{tabular}{ccccl}
\toprule
...
5 & 0 & 10 & 0 & no time \\
4 & 1 & 6 & 0 & no time \\
\textbf{3} & \textbf{2} & \textbf{3} & \textbf{1} & $\boldsymbol{3+1}$ \textbf{spacetime} \\
2 & 3 & 1 & 3 & $1+3$ (mirror) \\
1 & 4 & 0 & 6 & no space \\
0 & 5 & 0 & 10 & no space \\
\bottomrule
\end{tabular}
\end{center}
The $(3,2)$ split is the \emph{only} partition where
temporal curvature is unique (Newtonian gravity),
spatial curvature has 3 modes (3-dimensional geometry),
and all three curvature types coexist.
\end{corollary}

\subsection{Tetrahedron-vertex duality and time/space identification}

The same opposite-element logic extends to tetrahedra (3-faces).
Each tetrahedron has one opposite vertex whose character
determines the normal direction:

\begin{center}
\begin{tabular}{lccl}
\toprule
Tetrahedron & Opposite & Count & Interpretation \\
\midrule
$AAAB$ & $B$ vertex & 2 & timelike normal $\to$ constant-$t$ slice \\
$AABB$ & $A$ vertex & 3 & spacelike normal $\to$ constant-$x$ boundary \\
\bottomrule
\end{tabular}
\end{center}

$AAAB$ tetrahedra (opposite $= B$) have timelike normals
and define spatial slices --- this is why $AAAB$ generates
the time direction.
$AABB$ tetrahedra (opposite $= A$) have spacelike normals
and define spatial boundaries --- this is why $AABB$
generates the space direction.
Together with the edge--triangle duality (Theorem~\ref{thm:opposite}),
this establishes a complete
$k$-face $\leftrightarrow$ $(4-k-1)$-face duality with
matching type counts: $3 \leftrightarrow 3$ (spatial),
$2 \leftrightarrow 2$ (temporal), $6 \leftrightarrow 6$ (mixed).

\subsection{The $\delta_{\mathrm{TTT}} = 0$ theorem}
\label{subsec:ttt}

Within a single $(3,2)$ simplex, a $BBB$ hinge is
\emph{combinatorially impossible} ($\binom{2}{3} = 0$),
so $\delta_{\mathrm{TTT}}$ is undefined ---
not zero, but nonexistent.
In the full manifold $\partial(\Delta^5)$ with three
$B$-vertices, the unique $BBB$ hinge $\{B_1,B_2,B_3\}$
\emph{does} exist, and its deficit angle is identically zero:

\begin{theorem}[$\delta_{\mathrm{TTT}} = 0$ on $\partial(\Delta^5)$]
\label{thm:ttt}
...
\begin{proof}
The $BBB$ hinge is surrounded by three simplices
...
Pure temporal hinges either do not exist (per-simplex level)
or carry zero curvature (manifold level).
Gravity is generated exclusively by hinges with at least
one spatial vertex.
\end{corollary}

\begin{remark}
This is a two-level result:
\emph{(i)}~within any $(3,2)$ simplex, $BBB$ hinges are
combinatorially forbidden (Theorem~\ref{thm:opposite});
\emph{(ii)}~on $\partial(\Delta^5)$, the single $BBB$ hinge
has $\delta = 0$ by $A$-orthogonality.
Both mechanisms ensure that pure temporal curvature is absent.
This explains why the lapse function $N$ in the ADM formalism
is a constraint rather than a dynamical variable:
pure-temporal geometry is flat.
\end{remark}

Contrast with the $AAA$ hinge: $\delta_{\mathrm{AAA}} = 90°$
...
$3+1$ structure & $\binom{\nT}{2}:\nS\nT:\binom{\nS}{2} = 1:6:3$ & $3+1$ spacetime & \checkmark \\
\bottomrule
\end{tabular}
\end{center}
