...
## Phase 26 — ℕ 기준 타워: 루트는 그림자, 점화식이 정확한 형태

User: 시드가 자꾸 루트(`ℤ[√2]`, `ℤ[√5]`)로 나오는 건 `ℤ` 기준이라
그런가?  `ℕ` 기준으로 짜면 더 정확한 형태가 나올까?  **그렇다** —
`NaturalTowerForm.lean` (∅-axiom, 4 theorems):

  * `seeds_are_nonsquare_residues` — `√D`는 `D`가 `ℕ`의 완전제곱이
    아닐 때만 "루트"다.  radicand가 연속 제곱수 사이에 끼어 있어
    (`1²<NT<2²`, `2²<NS+NT<3²`) `ℕ`/`ℤ` 값이 그것을 제곱하지 못함 —
    surd는 **실패한 제곱근의 잔여물**.  `−NS`는 음수 (`m² ≥ 0 > −3`,
    `IntHelpers.mul_self_nonneg`) — 실근 자체가 없음.
  * `lucas_recurrence_natural_form` — `E₈` 시드 `√(NS+NT)=√5`의 `ℕ`
    정확형은 **Lucas 점화식** `L₀=NT, L₁=NS, L_{n+1}=NS·L_n−L_{n−1}` →
    `2,3,7,18,47` (순수 ℕ, √5 없음).  계수 = `NS = trace P`, 시드 =
    `{NT, NS}` — 원자 숫자가 곧 점화식 데이터.
  * `lucas_is_trace_P_pow` — `L_n = trace Pⁿ` (Möbius `P=[[2,1],[1,1]]`);
    `√5`는 `P`를 `ℝ`에서 대각화할 때 (고유값 `φ²,φ⁻²`)만 등장하는
    그림자.
  * `natural_tower_form` — 종합.

**답.**  `ℕ` 기준이 더 정확하다.  예외 구조의 실체는 **정수 행렬 `P`와
유한군 `2T,2O,2I`** (순수 counting 객체)고, `√D`는 그것들의 `ℝ`-trace
그림자 — `ℕ`이 품지 않는 제곱근의 잔여물.  `√(NS+NT)`는 Lucas 점화식,
`√NT`는 더블링 맵 `D(x)=x²−NT` (`TraceDoublingMap`)로 녹는다.  surd는
닫힌형(Binet) 그림자, 점화식이 finitary exact form.  이는 잔여물 엔진과
일치: 루트 = `ℕ`에서 제곱근이 실패한 자리의 잔여물.

## Phase 27 — 세 축 전부 surd-free: 정수 companion 행렬

Completes Phase 26 across all three axes.  `ThreeAxisRecurrence.lean`
(∅-axiom, 5 theorems): each seed `√D` is the eigenvalue shadow of an
integer companion matrix; over `ℕ`/`ℤ` there are no surds.

| axis  | matrix `M`        | min poly       | `trace Mⁿ`       | disc       | type      |
|-------|-------------------|----------------|------------------|------------|-----------|
| `2`   | `[[NT,1],[1,0]]`  | `x²−NT·x−1`    | `2,2,6,14,34`    | `NT²+4=8`  | Pell      |
| `3`   | `[[0,−1],[1,−1]]` | `x²+x+1` (Φ₃)  | `2,−1,−1` (per 3)| `−NS=−3`   | cyclotomic|
| `2+3` | `[[2,1],[1,1]]=P` | `x²−NS·x+1`   | `2,3,7,18,47`    | `NS+NT=5`  | Lucas     |

  * `axis_two_pell` / `axis_three_eisenstein` / `axis_two_three_lucas` —
    the three trace sequences + discs.  `M₃³ = I` (order `NS`, finite).
  * `disc_sign_is_field_type` — **the discriminant sign is the field
    type, hence the dynamics**: `disc > 0` (`8, 5`) real ⇒ unit group
    infinite ⇒ trace *grows* (Pell, Lucas); `disc < 0` (`−NS`) imaginary
    ⇒ unit group finite ⇒ trace *periodic* (`M³=I`, order `NS`).
  * `three_axes_surd_free` — synthesis.

**The trichotomy is now fully `ℕ`-native.**  Three integer matrices,
coefficients the atomic `{NT, NS}`, discriminants `{8, −NS, NS+NT} =
{8, −3, 5}` (the `Phase 19/23` field discs); two real/growing
(hyperbolic, `√2`/`√5`) and one imaginary/periodic (elliptic-finite,
`ω`, order `NS`).  The surds `1±√2`, `φ²,φ⁻²`, `ω,ω̄` are the `ℝ`-eigenvalue
shadows; the matrices are the exact form.  Answers the user end-to-end:
the `√D` are the `ℤ`/`ℝ`-frame's diagonalisation shadows; over `ℕ` the
exceptional structure is three integer recurrences — `disc`-sign-split
into the real (`P`-engine, growing) and imaginary (`ω`-engine, periodic)
worlds.
