...
- **A3.** `N(J) = J·J̄ = p` — the double sum collapsed by orthogonality / translation invariance.
  ...
  - **Remaining: the `J·J̄ = p` double sum.**  Substrate **DONE** — `EisensteinListSum` (generic
    ...
    4. **reindex** (NEXT) — `chiOmega_reindex` (DONE, `EisensteinJacobiReindex`) is the per-term
       ...
       `a,b∈{0,1}`).
    5. **the inner-sum collapse** (the genuine hard core).  **CORRECTION (verified on paper): the pure
       multiplicative `ℤ[ω]`-internal route (a) is CIRCULAR.**  The reindex `b=(a·s)` (built:
       `jacobi_inner_reduce`) reduces `J·J̄ = Σ_a χ_ω(1−a)·T(a)`, `T(a)=Σ_s χ̄_ω(s)χ̄_ω((1−as)%p)`; but
       a second substitution `s'=as` gives `T(a)=χ_ω(a)·J̄`, so `J·J̄ = Σ_a χ_ω(a)χ_ω(1−a)·J̄ = J·J̄`
       — a tautology.  **Multiplicative structure alone cannot prove `|J|²=p`; the ADDITIVE character
       is essential.**  (`|g(χ)|²=p` needs additive orthogonality `Σ_t ζ^{at}=0`, `a≠0`.)
       **REVISED ROUTE: (b) Gauss sums.**  Build the additive character / Gauss sum
       `g(χ)=Σ_t χ_ω(t)·ζ^t` over a cyclotomic carrier `ℤ[ζ_p]` (`ζ^p=1`); then
