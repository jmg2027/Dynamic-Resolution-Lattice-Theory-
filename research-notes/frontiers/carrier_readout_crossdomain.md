...
Rung 2 is complete: the residue ring `ℤ[ω]/(d)` is (a quotient of) `𝔽_p`.  Next rungs:
**(3)** the field structure / Fermat on `ℤ[ω]/(d)` ⟹ `α^{(p−1)/3} ∈ μ₃ mod d`, so the cubic character
`(α/d)₃` is well-defined into the `μ₃` above (welded, via the `reduce_to_int` rational reduction, to
