...
Next rungs:
**(2c)** derive `ω ≡ r (mod π)` unconditionally from the split `p = N(π)` + `EisensteinCubeRoot`
(`x²+x+1 ≡ 0 mod p`), making `reduce_to_int` unconditional; then the field/Fermat structure ⟹
`α^{(p−1)/3} ∈ μ₃ mod π`, so `(α/π)₃` is well-defined into the `μ₃` above;
