...
| 10 | 2026-06-05 | essay | The slash-reading atlas (one residue, many shapes) | A user-driven geometric exploration — "object = relation of two, recurse" rendered many ways — became a synthesis once three structural cells were closed ∅-axiom (SimplexOrthogonality, MetallicThreshold, K32Adjacency).  The essay ties the continuum / Δ^∞ / Stern–Brocot / K_{3,2} readings as facets of one slash, with the cross-frame punchline that the constants 3,5,φ sit at the forced count-minimum a=2 (= the det=1 glue).  Pattern: *an open-ended "render this geometrically" exploration that, once its spine closes in Lean, promotes to a synthesis essay mapping the readings + their constant-surfacing meta-thesis.* | `theory/essays/synthesis/slash_reading_atlas.md` |
| 11 | 2026-06-05 | essay | The readings re-derive the seed (atlas ↔ axioms) | After reading the seed/AXIOM docs against the geometric atlas, every found regularity resolved to a specific axiom fact: readings are folds out of the initial object (4.2); the collapse↔forced-golden dial is the from-below/from-above uniqueness (4.1/4.3); reached-by-none is the 1.0' diagonal; the convergent/periodic split is the 5.2 Nat/Bool self-reference; and the pure-combinatorics entry re-reaches ORIGIN's physical floor.  Pattern: *a broad exploration that, once mapped back onto the foundations, becomes a primacy-as-breadth demonstration essay — the survey IS the test of 7.1.* | `theory/essays/synthesis/readings_re_derive_the_seed.md` |
