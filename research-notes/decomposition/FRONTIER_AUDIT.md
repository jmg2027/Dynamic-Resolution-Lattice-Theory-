...
| `galois_correspondence.md` | A₅ perfectness `[A₅,A₅]=A₅`; general `isSolvable` predicate | **FIXED** | A₅-perfectness PROVEN (`A5Perfect.a5_perfect`/`a5_not_solvable`, 9/0 PURE). General `isSolvable`/`isPerfect` + derived series as iterated operator BUILT (`Linalg213/Solvable.lean`, 65/0 PURE): `perfect_nontrivial_not_solvable` (pure induction, no enumeration) unifies `solvable_S3'` (q=+1) and `a5_not_solvable'` (q=−1) as the two `ResidueTag` values (`solvability_two_poles`). Strictly-smaller residual: the general `commClosure` (subgroup *generated* by commutators) for non-closed commutator sets; closed-case (S₃/A₃/A₅) uses `step = commSet`/`commList`. |
...
3. **A general `isSolvable` predicate** — **BUILT** (`Linalg213/Solvable.lean`, 65/0 PURE): the derived series is now an iterated operator `derivedSeries step G k` with `isSolvable step triv G := ∃ k, SameSet (derivedSeries step G k) triv` and `isPerfect step G := SameSet (step G) G`; the headline `perfect_nontrivial_not_solvable` (perfect non-trivial group ⇒ not solvable, pure induction, no enumeration) unifies both poles — `solvable_S3'` (q=+1) and `a5_not_solvable'` (q=−1, via `a5_isPerfect` from `A5Perfect.a5_perfect`), tagged by the `ResidueTag` `±1` bit (`solvability_two_poles`). **Remaining residual (strictly smaller)**: a general `commClosure` — the subgroup *generated* by commutators with a proven generation step — for groups whose commutator *set* is not already closed; `derivedSeries` is `step`-parametric and the closed-commutator instances (S₃/A₃/A₅) supply `step = commSet`/`commList`.
