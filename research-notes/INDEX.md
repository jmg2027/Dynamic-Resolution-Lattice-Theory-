...
| `G164_holonomic_generator_constructive_boundary.md` | The generator is total-constructive on the autonomous (algebraic) class AND degree-1 (e): e has a total modulus `N(m,k)=k+2` (`ExpLog/EulerModulus`, `eHolonomicReal`) — the "LEM wall" is a rate-free artifact, not transcendence.  Open: π (deg 4) + general | **active** finding |
| `G165_modulus_rate_frontier.md` | The criterion for a FREE total modulus: `tail_i·k·d_i < 1` (rate beats denominator-gap quantum).  e meets it; π-via-Wallis fails it (needs `μ(π)`); the general generator = abstract `EulerModulus` over `(a,d)` + this hypothesis | **active** finding |
