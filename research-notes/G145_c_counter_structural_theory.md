...
### Conjecture 4 — Möbius lattice sampling thesis

Every bipartite multigraph `K_{NS, NT}^{(c)}` corresponds to a specific
POINT in the Möbius P lattice generated by `[[2,1],[1,1]]`-iterates from
seedZero / seedInf.

Specifically:
- `(NS, NT)` lies in `Z[Pseq seedZero] ⊕ Z[Pseq seedInf]` (additive
  combinations of orbit points).
- The c-multiplicity is a "Layer index" orthogonal to the lattice
  position.

Generalization claim: the Möbius P matrix is the universal generator
for ALL bipartite multigraph atomic signatures, not just the canonical
(3, 2) anchor.

**Open**: formalize the lattice decomposition for arbitrary `(NS, NT)`.
