...
## §17 NativeGuard — vocabulary-level enforcement
...
primitives.  Anything else (Mathlib, Std4, Classical.*, etc.) fails
elaboration.

`lean/E213/Meta/Tactic/NativeGuard.lean` provides three primitives,
mirroring PureGuard's design:

  `#guard_native <name>`                      — fail if any directly-
                                                used constant is non-
                                                213-native
  `#guard_native_with <n> allowing ["X","Y"]` — extend allow-list with
                                                additional namespace
                                                prefixes (sealed cases)
  `@[native213]` attribute                    — declaration-time
                                                native enforcement

Default allow-list:
  - `E213.*` (project namespace)
  - Lean kernel types: `Nat`, `Bool`, `Prop`, `Type`, `Sort`, `String`,
    `Char`, `Unit`
  - Logical: `True`, `False`, `And`, `Or`, `Not`, `Eq`, `Iff`, `Ne`
  - Containers: `Prod`, `PProd`, `Subtype`, `Sum`, `Option`
  - Decidability + numerics: `Decidable`, `decide`, `OfNat`, arithmetic
    typeclasses, `Min`, `Max`
  - Init utilities: `id`, `Function`, `congrArg`, `congrFun`, `letFun`,
    `cond`, `Inhabited`, `Repr`, etc.
  - Raw-prefix matches for autogenerated names: `inst*`, `_aux*`,
    `_private*`, `_eqn*`

Together with PureGuard, the catalog now has **two complementary
build-time guarantees**:

  · **PureGuard**   — zero kernel-axiom dependencies (∅-axiom)
  · **NativeGuard** — zero non-E213 vocabulary dependencies

A declaration passing both is **213-pure in the strongest sense**:
no axioms, no external vocabulary.  `NativeGuardTest.lean` runs
`#guard_native` against 19 catalog declarations; each emits
`✓ '<name>' is 213-native` at build time.

Together with the third self-correction (heterogeneous-target
dependent matching → structural noise), 213 now has a complete
mechanical *self-defense system*:

  · ∅-axiom regime    — propext / Quot.sound / Classical refused
  · 213-native regime — non-E213 vocabulary refused
  · Structural noise  — heterogeneous type families refused

Three orthogonal axes of strictness, all enforced at build time, all
surfaced as `error` rather than report.  The catalog is no longer
just *described* in Lean — it is *defended* in Lean.
