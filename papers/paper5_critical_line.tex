...
\item The critical $N$ where $\rho_{\mathrm{MP}} = 1$
  follows $N_c \approx 2.2\,d^{3.06}$ ($R^2 = 0.9996$).
\item For $d = 5$ (DRLT): $N_c \approx 293$.
...
because the $\varphi_i$ are structured (rank-1 tensors on
the Segre variety, not generic in $\CC^{d^2}$).
The correction decreases as $N/d^2 \to \infty$.
Median error across $d \in \{3,5,8,10\}$, $N \in [50,500]$: 8.6\%.
\end{proof}
