...
\end{equation}
Observed: $206.7682838$.  Error: $\mathbf{0.48\;\text{ppb}}$ (parts per billion).  The convergence is super-geometric: the three correction stages contribute $-182$ ppm, $-25$ ppm, $-1.3$ ppm, with residual ratio $0.125 \to 0.050 \to 0.0005$.
...
\begin{remark}
The bare value $\alpha_\text{GUT}^6 \approx 2.07 \times 10^{-10}$ slightly exceeds the current nEDM bound $|\theta| < 1.8 \times 10^{-10}$ (90\% CL). The residual cancellation from $\arg\det(Y_u Y_d)$ may reduce the physical value below the bound, but this cancellation is not yet derived from first principles. This remains an open problem.
\end{remark}
...
\begin{equation}
N_* = (d^2 - \log_d(c \cdot d_S)) \times \ln d \approx 37.9 \times 1.609 \approx 61.
\end{equation}
