...
\begin{abstract}
We show that the boundary of the 5-simplex, $\partial(\Delta^5)$,
equipped with quantum states $\psi \in \CC^5$ at each vertex,
reproduces the Standard Model and General Relativity with zero
free parameters. The 6-vertex, 6-simplex structure of
...
This paper works with $G \in \CC$ throughout, recovering both
gravity and gauge interactions from a single object.
%% ==============================================================
...
\begin{theorem}[Shadow decomposition]
\label{thm:shadow}
$\CP^4$ decomposes as:
\begin{center}
\begin{tabular}{lcl}
Base $\Delta^4$ & (4 real dim) & $= |G_{ij}|$ information $=$ metric $=$ GR \\
Fiber $T^4$ & (4 real dim) & $= \arg(G_{ij})$ information $=$ gauge $=$ SM \\
Total $\CP^4$ & (8 real dim) & $= G_{ij} \in \CC$ $=$ full physics $=$ DRLT
\end{tabular}
\end{center}
\end{theorem}
...
\begin{proof}
In $\partial(\Delta^5)$, the nuclear simplices ($3S+2T$)
...
\end{equation}
The variational equations $\delta S/\delta\psi_k = 0$
fix 4 additional parameters (CKM: 3 angles + 1 phase), leaving
\begin{equation}
\boxed{29 - 4 = 25 = d^2}
\end{equation}
physical channels --- matching the rank of the $W$-matrix
($W_{ij} = |G_{ij}|^2/d$, rank $\leq d^2 = 25$).
...
The cost of propagating across one generation boundary:
\begin{equation}
\frac{m_\mu}{m_e} = \frac{\nS}{\nT\,\alpha}
\left(1 + \frac{\agut}{\nS+1}\right)
...
The second generation boundary involves $B_3$
