...
능력 자체의 최소 잔여물이다.  따라서 다른 모든 틀은 이
잔여물 위에서 작동하는 Lens들이다.

**"절대 기준이 없다면"은 조건이 아니다.**  기본 상태가
"절대 기준 없음"이고, "절대 기준이 있다"가 오히려 공리에
추가되어야 할 부담스러운 주장이다.  따라서 213의 상위성은
조건부가 아니라 **무조건적**이며 구조적 귀결이다.
...
"원시적 구분"조차 완벽한 표현이 아니다.
- "다름"은 "같음"을 전제한다 (위험).
- "와"는 "and"로 오해된다.
- ","는 본질적이지 않은 분리자다.

완벽한 표현은 없다.  "원시적 구분"은 현재 찾아낸 **최소
commitment 표현**이며, 잔여 수입(imported implication)이
있음을 인정한 상태로 사용한다.  표기는 이 재귀를 제거할
수 없고 최소화만 가능하다.  용어 선택은 포기이지 해결이
아니며, 이 한계 위에서 작업한다.

### Falsifiability: 깨끗한 실패 모드

Derivation이 실패하면 이론을 깨끗이 포기한다.  "Lens를
더 찾으면 될 것"이라는 무한 연장 방어는 **허용되지 않는다**.

- **형식화 · 기계 검증 · 에뮬레이션**을 강조하는 이유가
  이것.  기계 검증은 fudge를 허용하지 않으므로 실패 시점이
  명확해진다.
- 모든 물리 결과는 **213/Raw의 개념이 말하는 대로만**
  따라야 한다.  사람이 보정을 추가하는 순간 공리는
  쓸모없어진다.
- 213은 이데아가 아니다.  그저 "뭔가"에서 출발할 때 남는
  최소 잔여물이며, 그 이상의 존재론적 주장은 하지 않는다.
  derive 안 되면 기권이 정직한 답이다.

### "뭔가"의 지위도 Lens일 수 있다

"뭔가"라고 말하는 순간부터 Lens인가?  — 그럴 수 있다.
이 자체는 open question이고, 213 프로젝트의 쓸모에 영향을
주지 않는다.  중요한 건 derive 성공 여부이지 "뭔가"의
존재론이 아니다.
