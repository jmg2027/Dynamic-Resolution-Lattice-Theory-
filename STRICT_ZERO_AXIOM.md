...
`E213.Lib.Math.PolyZ` — **16 PURE / 0 DIRTY**.  **Integer-coefficient polynomials** (the `ℕ`-valued `Polynomial213` cannot carry the signed `XI − M`).  `PolyZ := List Int` (low-to-high), Horner `eval`, and `C`/`Xp`/`addP`/`negP`/`scaleP`/`shiftP`/`mulP`/`coeff`.  ★ **eval soundness** — each operation commutes with evaluation: `eval_C`/`eval_Xp`/`eval_addP`/`eval_scaleP`/`eval_shiftP`/`eval_negP`/★★`eval_mulP`; `PolyZ` is a commutative-ring reflection of `Int`.
