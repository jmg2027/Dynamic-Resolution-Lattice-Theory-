...
| Module | PURE | Highlights |
|---|---|---|
| `E213.Lib.Math.Cohomology.Bipartite.V33EnrichedParametricDualSpan` | 16 | `psi_layer_flat` (Nat → Bool view of 9 face values); ★★★★ `psi_layer_eq_psiNatPos` (rfl-bridge to abstract XOR fold); ★★★★ `psi_layer_linear` (F₂-linearity over pointwise XOR); `weighted_e_sum` (`Σ_m b m · e_face_layer m`); ★★★★ `psi_layer_weighted_e_sum` (ψ-vector hits any target); `psi_vector_surjective` (Ψ surjectivity); `psi_projection` / `psi_residual` (canonical splits); ★★★★ `psi_residual_kills_all_psi`; ★★★★★ `psi_canonical_decomposition`; `InPrimaryCupSpanPlusBoundary` (F₂-span of PRIMARY cup-products `starS ∪ β`, `α ∪ incidT` + coboundaries); ★★★★★ `psi_layer_arbitrary_cup_not_kill` (counter-example: ψ doesn't kill arbitrary single-edge cup, so FULL cup-image span = entire FaceVal — justifies the PRIMARY restriction); ★★★★★★ `primary_cup_span_soundness_conditional` (`InPrimary ⊆ joint ψ-kernel` under Direction B's arbitrary-m kill hypotheses); ★★★★★★★ `primary_cup_span_soundness_c1` (★ **UNCONDITIONAL** soundness at c=1: at single layer the conditional collapses since `Fin 1` forces `m = ⟨0, _⟩`); ★★★★★★ `codim_upper_bound_conditional` (conditional upper bound under joint-ψ-kernel ⊆ `InPrimary` hypothesis); ★★★★★★★★ `parametric_dual_span_capstone` (capstone: c ψ-discriminators span the dual of `EnrichedFaceVal c / InPrimaryCupSpanPlusBoundary`) |

**Session grand total after V33EnrichedParametricDualSpan**: 393 + 16 = **409 PURE / 0 DIRTY**.
