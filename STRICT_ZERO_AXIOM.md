...
`tools/scan_all_axioms.py`:
  ...
  · **No `Classical.choice` and no `Lean.ofReduceBool` (`native_decide`) in any
    ...
    The only `Classical.choice` carriers are the **command-elaborator defs
    `E213.Tactic.elab{Verify,Derive}Conjugation`** (the `: CommandElab` definitions
    generated by the `derive_conjugation` / `verify_conjugation` macros), which
    inherit `propext + Classical.choice + Quot.sound` via the `Lean.Elab.Command`
    monad — sealed plumbing per category (a), not math content. (Audit note: the
    sibling `Meta.Tactic.NativeGuard` decls carry only `propext` / `Quot.sound`, not
    `Classical.choice`; `QuadExtension` / `DeriveConjugationCodomain` have 0 DIRTY
    decls in isolation — the `Classical.choice` lives in the `elab*` defs above,
    which is why the scanner now seals them by decl name.)
...
Standing status: **0 real DIRTY** — every DIRTY declaration is one of the
sealed-by-design entries below, waived under the categories above.

**Census-method caveat (audited 2026-06-22 — read before citing a count).** The
authoritative checker is **per-module `tools/scan_axioms.py <module>`** (reliable;
e.g. `Nat213.Irreducible` → 18/18 PURE). The whole-corpus **`scan_all_axioms.py
--csv` batch mode is currently *unreliable as a total*** — a single run resolved
only **4177 of the ~20,429 top-level `theorem`/`lemma`/`def`** declarations
(`grep -rE '^(protected )?(theorem|lemma|def) ' lean/E213 | wc -l`), the rest
silently dropped to per-module probe timeouts / name-resolution failures. So:
  · **True declaration count** ≈ **20,429** top-level `theorem`/`lemma`/`def`
    (grep, reliable); the older "18,845" figure is in the right ballpark, slightly
    stale (the corpus has grown). Do **not** cite a precise whole-corpus PURE
    total from `--csv` until the batch probe is fixed.
  · **0 real DIRTY** holds on the resolved set (4137 PURE / 0 real / 40
    sealed-by-design in the audited run) and on every per-module spot check; the
    sealed set is enumerated below.
  · One **false** "real DIRTY" was found and fixed: the command-elaborator defs
    `E213.Tactic.elab{Verify,Derive}Conjugation` get *attributed to the module that
    invokes the command macro* (e.g. a `Cauchy.*` file), which module-based sealing
    missed. `scan_all_axioms.py` now also seals by decl name (`SEALED_DIRTY_DECLS`,
    prefix `E213.Tactic.elab`) — same CommandElab category (a), not math content.
  · **Open tooling frontier**: the `--csv` batch probe's ~80% dropout is a genuine
    gate-integrity gap (a comprehensive gate that silently skips most decls cannot
    guarantee the standard — cf. "Gate vindication" below). Fixing the probe
    (name-qualification + per-decl timeout handling) is the precise next tooling
    task; until then, trust per-module scans, not the `--csv` aggregate.
