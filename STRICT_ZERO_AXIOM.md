...
`E213.Lib.Math.Algebra.Linalg213.PermSign` — **4 PURE / 0 DIRTY**.  Toward sign-multiplicativity `psign(σ∘τ) = psign σ·psign τ` (the keystone for `det Mᵀ = det M`), the **position-swap action** machinery: ★ `map_swapAt` (`map` commutes with a position-swap) ⟹ ★ `composeList_swapAt` (`σ ∘ swapAt k τ = swapAt k (σ∘τ)` — the inductive step's geometry); ★ `psign_swapAt` (an adjacent swap of distinct entries negates `psign`, via `swapAt_prefix`+`psign_swap_prefix`); ★ `swapAt_mem_perms` (`perms` closed under a position-swap).  The remaining engine for `psign_mul` (bubble-sort induction: adjacent-descent existence + directed inversion-decrease + `inversions τ = 0 ⟹ τ = iota n`) is the documented next step.
