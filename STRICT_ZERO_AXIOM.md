...
`E213.Lib.Math.Algebra.Linalg213.DetTranspose` — **5 PURE / 0 DIRTY**.  Toward the transpose determinant `det Mᵀ = det M` (the payoff of `PermSign.psign_mul`).  **§1**: `nodup_map_restrict` (cnt-`Nodup` under a map injective *on the list's elements*), `perms_contains` (a permutation of `iota n` holds every value `< n`), and ★ `invPerm_mem_perms` — **the inverse of a permutation is a permutation** (`invPerm σ ∈ perms n`, via `lperm_of_nodup_mem_iff` with the position/value membership bijection).  **§2**: `psign_iota` (`psign (iota m) = 1`) and ★★★ `psign_inv` — **`psign (σ⁻¹) = psign σ`**, a one-liner from `psign_mul` (`psign σ·psign σ⁻¹ = psign (σ∘σ⁻¹) = psign (iota n) = 1` in `{±1}`).  Phase 2 (`det Mᵀ = det M` itself: reindex `perms` by `invPerm` + the product-reindex) is the next step.
