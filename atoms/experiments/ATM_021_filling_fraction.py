"""
ATM_021: Subshell Filling Fraction
Joint research by Mingu Jeong and Claude (Anthropic)

Key insight: as a subshell fills, its screening interpolates
from σ_shell (empty, exposed) to σ_df (complete, core-like).

  σ(n_fill) = σ_shell + (n_fill/N_max) × (σ_df - σ_shell)

Physical meaning: each electron fills a "geometric slot" in ∂(Δ⁵).
When all N_max slots are filled → closed subshell → screens like core.

Applied to f-block (N_max=14): Yb error +23% → +0.4%
"""
import sys, os
...
class FillingFraction(Experiment):
    ID = "ATM_021"
    TITLE = "Subshell Filling Fraction"
    ...
    def test1_concept(self):
        self.log(f"\n  {'='*55}")
        self.log(f"  Test 1: Filling fraction concept")
        self.log(f"  {'='*55}")
        self.log(f"\n  sigma(n_fill) = sigma_shell + (n/N_max)(sigma_df - sigma_shell)")
        self.log(f"  sigma_df = 1 - alpha_GUT = {S_DF:.5f}")
        sf_base = sigma_shell(4, 6)
        self.log(f"  sigma_shell(4->6) = {sf_base:.5f}")
        self.log(f"  Delta = {S_DF - sf_base:.5f}")
        self.log(f"\n  f-block filling (N_max=14):")
        for nf in [1, 7, 10, 14]:
            sf = sigma_fill(nf, 14, 4, 6)
            self.log(f"    n_f={nf:2d}: sigma={sf:.5f}")
        self.check("Filling fraction interpolates correctly",
                    abs(sigma_fill(14, 14, 4, 6) - S_DF) < 1e-10)
